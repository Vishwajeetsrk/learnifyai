...
| :------------- | :------------------------------------------------------------------------------------------------------------------- | :---------------------- |
| **JavaScript** | ![JavaScript](https://img.shields.io/badge/JavaScript-ES6+-F7DF1E?style=flat-square&logo=javascript&logoColor=black) | Node.js 20.17           |
| **TypeScript** | ![TypeScript](https://img.shields.io/badge/TypeScript-5.x-3178C6?style=flat-square&logo=typescript&logoColor=white)  | TypeScript 5.6          |
| **Python**     | ![Python](https://img.shields.io/badge/Python-3.12-3776AB?style=flat-square&logo=python&logoColor=white)             | CPython 3.12.7 (Stable) |
| **Java**       | ![Java](https://img.shields.io/badge/Java-22-ED8B00?style=flat-square&logo=openjdk&logoColor=white)                  | OpenJDK JDK 22          |
| **C++**        | ![C++](https://img.shields.io/badge/C%2B%2B-17%2F20-00599C?style=flat-square&logo=c%2B%2B&logoColor=white)           | GCC 14                  |
| **C**          | ![C](https://img.shields.io/badge/C-ISO-A8B9CC?style=flat-square&logo=c&logoColor=black)                             | GCC 14 C                |
| **C#**         | ![C#](https://img.shields.io/badge/C%23-Mono-512BD4?style=flat-square&logo=dotnet&logoColor=white)                   | Mono 6.12               |
| **Go**         | ![Go](https://img.shields.io/badge/Go-1.23-00ADD8?style=flat-square&logo=go&logoColor=white)                         | Go 1.23.2               |
| **Rust**       | ![Rust](https://img.shields.io/badge/Rust-1.82-000000?style=flat-square&logo=rust&logoColor=white)                   | Rust 1.82.0             |
| **PHP**        | ![PHP](https://img.shields.io/badge/PHP-8.3-777BB4?style=flat-square&logo=php&logoColor=white)                       | PHP 8.3.12              |
| **Ruby**       | ![Ruby](https://img.shields.io/badge/Ruby-3.x-CC342D?style=flat-square&logo=ruby&logoColor=white)                    | Ruby 4.0.2              |
| **Swift**      | ![Swift](https://img.shields.io/badge/Swift-6.0-FA7343?style=flat-square&logo=swift&logoColor=white)                 | Swift 6.0.1             |
| **Kotlin**     | ![Kotlin](https://img.shields.io/badge/Kotlin-2.0-7F52FF?style=flat-square&logo=kotlin&logoColor=white)              | Kotlin 2.0.21           |
| **Dart**       | ![Dart](https://img.shields.io/badge/Dart-3.5-0175C2?style=flat-square&logo=dart&logoColor=white)                    | Dart 3.5.3              |
| **Bash**       | ![Bash](https://img.shields.io/badge/Bash-Shell-4EAA25?style=flat-square&logo=gnubash&logoColor=white)               | Bash 5.2                |
| **SQL**        | ![SQL](https://img.shields.io/badge/SQL-SQLite-003B57?style=flat-square&logo=sqlite&logoColor=white)                 | SQLite 3.46.1           |
| **Zig**        | ![Zig](https://img.shields.io/badge/Zig-0.13-F7A41D?style=flat-square&logo=zig&logoColor=white)                      | Zig 0.13.0              |
| **Julia**      | ![Julia](https://img.shields.io/badge/Julia-1.10-9558B2?style=flat-square&logo=julia&logoColor=white)                | Julia 1.10.5            |
| **Lua**        | ![Lua](https://img.shields.io/badge/Lua-5.4-2C2D72?style=flat-square&logo=lua&logoColor=white)                       | Lua 5.4.7               |
| **Haskell**    | ![Haskell](https://img.shields.io/badge/Haskell-9.10-5D4F85?style=flat-square&logo=haskell&logoColor=white)          | GHC 9.10.1              |
| **Elixir**     | ![Elixir](https://img.shields.io/badge/Elixir-1.17-4B275F?style=flat-square&logo=elixir&logoColor=white)             | Elixir 1.17.3           |
| **Nim**        | ![Nim](https://img.shields.io/badge/Nim-2.2-FFE953?style=flat-square&logo=nim&logoColor=black)                       | Nim 2.2.10              |
| **R**          | ![R](https://img.shields.io/badge/R-4.4-276DC3?style=flat-square&logo=r&logoColor=white)                             | R 4.4.1                 |
...
- ✅ **Admin Payment Gateway Switch**: New `site_settings.payment_gateway` key (`razorpay` | `cashfree`, seeded to `razorpay`) — `createSubscription` reads it at runtime and routes to Razorpay Native Subscriptions or the Cashfree fallback; Admin → Content Manager → Site Settings gets a proper dropdown. Course purchases stay wallet-based (Cashfree/Razorpay top-ups) as designed.
- ✅ **Legal Acceptance for Creators & Coaches**: `/apply-creator` and `/apply-coach` now display the platform policies (Privacy / Terms / Refund) with explicit acceptance text before submit, and the Admin application review dialog shows a **Legal** panel linking the same policies when approving creators/coaches.
- ✅ **Course Covers v3 — Real Brand Icons**: New `CourseCoverImage` component renders official brand logos as tiles wherever a cover is missing (slug → logo map incl. PowerPoint, VS Code, GitHub, ChatGPT) and was swapped into Courses index (main + mini cards), Recommended Courses, Dashboard, Creator pages and user profiles — replacing the old `brokenImgs`/`markBroken` icon behaviour; covers redesigned with larger 132px logo tiles + `feDropShadow` tile shadow, cleaner composition, and the official PowerPoint mark added. `template-mastery` cover DB path fixed. Apple Pay `.well-known` response is served with `text/plain`, `no-cache` and CORS headers so payment domains verify.
- ✅ **TypeScript & Build Clean**: `tsc --noEmit` exit 0.

### v5.8.1 (August 2026) — Creators & Coaches Directory, Razorpay Native Subscriptions & Learning UX Batch

- ✅ **Creators & Coaches Public Directory**: `/coaches` and `/creators` are now real DB-driven pages (migration `20270812000001_creator_directory.sql`) — `creator_applications` gained structured `role` ('coach'|'creator'), `bio`, `hourly_rate`, `featured`, `avatar_url` (legacy coach rows auto-backfilled), public SELECT policy for approved entries, and `supabase_realtime` publication. A new `public_directory_entries` view exposes approved entries with profile names/avatars (bypasses the private `profiles` table), plus an anon "view published courses" policy for live creator course counts. Coach cards show Verified + Featured badges, bio and ₹/hr rate; "Book Session" deep-links to the coaching dashboard.
- ✅ **Admin Application Review Upgrade**: Approvals section in Admin now live-updates via a realtime channel, and the review dialog is a full **editor** — Display Name (→ `profiles.full_name`), Expertise, Public Bio, Hourly Rate (coaches), Avatar URL, Featured toggle, Admin Notes — with Save Changes, Approve and Reject. Apply forms (`/apply-coach`, `/apply-creator`) now submit structured rows instead of motivation-string hacks.
- ✅ **Razorpay Native Subscriptions**: Plans API sync replaces Cashfree for recurring billing — `createSubscription` creates real Razorpay plans + hosted subscription links (`short_url` redirect flow), `razorpay_plan_id` + `razorpay_subscription_id` persisted, and a new `/api/webhooks/razorpay-subscription` handler (HMAC-verified with webhook secret) manages `subscription.activated/charged/cancelled/halted/completed` plus `subscription.pending` (marks `past_due`, logs retry events). `verify-payment` now validates subscription-based signatures (`payment_id|subscription_id`) alongside order-based ones, and Razorpay domains are authorized in the CSP.
- ✅ **Past-Due Payment Alert**: New `SubscriptionPaymentAlert` on the Billing page — when a subscription is `past_due` users get an amber banner with **Update Payment Method** (Razorpay `subscription_card_change` checkout, supports card/UPI/eMandate) and an Auto-Retry notice; billing's subscription query now loads `past_due` rows (was active-only).
- ✅ **Learning Surface Cleanup**: Removed the English/हिन्दी language switcher from course player pages (content is single-language; switcher now only shows on app shell + marketing), leaving Theme + Listen controls. Studio project pages gained a **Run ▶ / Live Render** flow — HTML/CSS projects render as `srcDoc` inside the sandbox with a Refresh action, JS projects execute via the sandboxed `executeCode` VM with an in-page console drawer — plus an **AI Helper** sheet (explain/fix/optimize/complete against the editor code via OpenRouter) and real tech-stack badges (React, Tailwind, TypeScript, HTML, CSS, JS) in the sandbox header.
- ✅ **Course Resources (PDF / Cheat Sheet / Content)**: New `course_resources` table (migration `20270812000002_course_resources.sql`, applied live) with anon-read for published courses and creator/admin management policies. Creator Studio and Admin → Course System gain a **📎 Resources** manager (upload to the `media` bucket, type chips — PDF/Cheat Sheet/Notes/Image/Video/Content, add/rename/delete/download), and the course player's Resources tab shows them alongside assignments/materials.
- ✅ **Course Covers v2**: Cover generator redesigned — larger official brand logos on a bigger white tile with a brand-colored ring and glow (new official **PowerPoint** logo added, used on the Word & PowerPoint cover), modern `Inter/Segoe UI` font stack, tighter 42px titles, refined topic bullets and chips — regenerated for all 12 courses.
- ✅ **Stale-Chunk Resilience**: `GlobalSupportAgent` lazy-load now retries once and degrades gracefully to null instead of crashing the shell when a deploy leaves stale asset chunks (the `support-agent.functions-*.js 404` class of errors).
- ✅ **TypeScript & Build Clean**: `tsc --noEmit` exit 0.

### v5.8.0 (August 2026) — Branded Certificates, Course Covers Redesign & Platform Ops Batch

- ✅ **Certificate Template Cleanup — 204 → 8 Curated Designs**: Removed 160 legacy Canva-exports ("01-UIUX-Design"…'08-Data-Structures-2" folders, avg 574KB each) plus five fake-brand clones (Google/Coursera/Microsoft/Claude/Codex) from the repo and database (`git rm` ≈444MB pack weight; migration `20270811000002_cert_template_cleanup.sql`). Created **8 lightweight Learnify-branded SVG backgrounds** in `public/templates/learnify/*.svg` (royal navy & gold, emerald prestige, ivory academic, modern minimal, onyx luxury, aurora gradient, burgundy classic, corporate blue) and rewired `canva-cert.functions.ts` to a curated manifest (`CURATED_TEMPLATES`) with `learnifyLogo` + `qrCode` designer fields — no more fake-brand seeds.
- ✅ **Certificate DB Hygiene**: `certificate_templates` deduped 69 → 18 (single default "Learnify Official (Navy + Gold)", all `logo_url='/logo.png'`); 3 courses re-pointed to curated templates (Python → emerald-prestige, Figma → modern-minimal, ChatGPT & Claude → aurora-gradient); designer preview logo default fixed from the deleted `Logo Learnify AI.png`.
- ✅ **Course Covers Redesign**: `scripts/gen-course-covers.cjs` now renders **real brand logos** (`public/course-logos/*.svg` — official HTML/CSS/JS/Python/Excel/Word/Power BI/Figma/Google/Java/VS Code shapes, plus new ChatGPT, Claude & GitHub mark SVGs) on white tiles, a **"What You'll Learn" topic list** with colored dot bullets, FREE/level/lesson chips, and the inverted-white Learnify logo — replaces the old letter-glyph art on all 12 covers.
- ✅ **Invoice Designer Fixes**: Public `/verify/invoice/:id` hub now works for anonymous visitors (new policy "Anyone can verify invoice by number" — mirror of certificates; migration `20270811000003_public_invoice_verify.sql`), QR verify URL no longer hardcoded to the wrong domain (uses `window.location.origin`), verify page adds a **Download PDF** action, honest loading copy, and the designer's duplicate `setSaving` + stale `learnifyai.com` copy fixed.
- ✅ **Content Manager Bug Batch**: `yearly_price` now actually persists (three stale "column may not exist" strips removed — admin update, plan seed, `savePlan`); `max_courses` can be set to 0 (was coerced to -1); cohort inserts get the real `creator_id` (was `""`, a broken FK); "Seed Default FAQs" is now dedupe-guarded against repeated clicks; removed ~1,100 lines of dead certificate-manager code + the orphaned "Bulk Issue" tab.
- ✅ **Leaderboard Prizes Admin UI**: New **Leaderboard Prizes** tab in Content Manager (prize CRUD for weekly/all-time top-3, emoji picker, reward types, enable toggle, claims inbox with mark-claimed/revoke). Cron now also awards the **Hall of Fame (all-time)** prizes once; `claimPrize` AI-credit grants fixed (upsert used `onConflict: user_id` — the old `.eq()` was a no-op so credits were silently lost).
- ✅ **XP Store Real Rewards**: Purchases no longer just recorded — "Resume Builder Premium" grants 365 days of premium resume and "500 Extra AI Credits" actually credits the balance server-side; purchase order reversed (record → deduct) so a failed insert never costs XP; localStorage perk markers written so the "10% Discount Active" badge works.
- ✅ **Concept Graphs Fixed**: "Regenerate" actually regenerates (was stuck returning the cached map — `regenerateConceptGraph` was never called); AI JSON parsing hardened (invalid/empty maps error out instead of silently showing an empty graph, edges referencing missing nodes dropped); regenerate button now shows a spinner and failures surface a "Try Again" state.
- ✅ **TypeScript & Build Clean**: `tsc --noEmit` exit 0.

### v5.7.0 (August 2026) — Premium Course Covers & Curriculum Expansion

- ✅ **Premium Branded Covers for All 12 Courses**: New 800×450 covers generated by `scripts/gen-course-covers.cjs` into `public/course-covers/<slug>.svg` — per-course accent gradients, glyph ring, title + tagline, FREE · level · lesson-count badges, and the **official Learnify AI logo** (embedded PNG, inverted white on the dark art, matching the app's dark-mode treatment). Migration `20270810000003_course_covers.sql` points every course at its new cover (using the existing cover-image feature, now with consistent premium branding).
- ✅ **HTML & CSS Essentials 6 → 17 lessons, 2 → 4 modules** (migration `20270810000004_html_css_curriculum.sql`): new lessons on Lists/Links/Images, Forms & Buttons, Box Model in Action, Responsive Design, plus the **Build Real Projects** module (Business Card, Space Exploration Page, Birthday Gift Website, NFT Landing Page) and **Accessible & Essential CSS** (Text Contrast & Color, Semantic Layouts & ARIA, Portfolio Site Project) — every lesson with callouts, tables, diagrams, quiz blocks, and memory-tested "First Understanding / Suggestions and Tips" framing, 4 new AI-graded exercises, 2 new assignments.
- ✅ **JavaScript Zero to Pro 6 → 16 lessons, 2 → 4 modules** (migration `20270810000005_js_curriculum.sql`): Conditions & Truthiness, Loops & Arrays, Events & Click Handlers, Templates & innerHTML, then **Build with JavaScript** (Counter App Project, Basketball Scoreboard Solo Project, Blackjack Game Logic) and **Modern JavaScript** (Chrome Extension Essentials, Promises & Error Handling, Build a Meme App) — 5 new exercises, 3 new assignments.
- ✅ **"Today in the Industry" Modules** (migration `20270810000006_industry_lessons.sql`): 10 practice-first lessons across the remaining courses — Copilot & AI Pair Programming (VS Code + Git), Claude & GPT at Work (ChatGPT/Claude), Python + AI Tooling, Gemini in Workspace (Google Workspace), AI in Spreadsheets (Excel/Sheets), Power BI & AI Auto Insights, Java in 2026, Word/PPT with Copilot, Figma AI, and AI Site Builders vs Templates (Template Mastery) — each with callouts, a quiz block and today's job-market context.
- ✅ **109 total lessons across 12 courses** (was 78), 27 exercises (was 18), 15 assignments (was 9), 36 modules (was 22) — project-per-module cadence and memory-friendly micro-content per the Scrimba curriculum scan in `Learn and Developer`.
- ✅ **Production SSR Build Fix**: The site was returning 500 on every page after a prebuilt deployment — the local `.env` carried `NODE_ENV=development`, which flipped Vite's JSX transform to the dev runtime (`jsxDEV`) while bundling React's production runtime (where `jsxDEV` is intentionally `undefined`), crashing `renderToReadableStream`. Fixed by removing the stray `NODE_ENV` from `.env` and forcing `--mode production` in the build script (`package.json`) — `tsc`, local smoke test and live checks all pass.
- ✅ **Maintenance Page**: Beautiful standalone `public/maintenance.html` + animated `public/maintenance.svg` (spinning gears, pulsing lightning bolt, starfield, "Maintenance Time") — ready to serve anytime the site needs downtime.
...
- 💳 **RESPONSIVE PLAN BREAKDOWN CARDS**: Upgraded Plan Breakdown list items (*Enterprise*, *Pro*, *Free*, *Career Pro*) into responsive cards with mobile-optimized metric badges (*Active*, *Cancelled*, *Expired*, *MRR*) scaling cleanly across 320px–1440px viewports.
...
- 🧾 **GST TAX INVOICE & EMAIL AUTOMATION**: Automated Cashfree & Razorpay payment webhook processing with 18% GST calculations (CGST 9% + SGST 9%, HSN/SAC 998431), luxury canvas PDF rendering (`downloadInvoicePdf`), and instant email invoice delivery via Resend/SMTP (`sendInvoiceCreatedEmail`).
...
- 🔗 **SOCIAL LINKS & LINKEDIN NORMALIZATION**: Fixed URL duplication bug in `normalizeLinkedin` and `normalizeGithub` (`https://linkedin.com/in/www.linkedin.com/in/...`), ensuring clean canonical social profile links.
- 🛠️ **PRESERVED SHORT TECH TOKENS**: Fixed over-aggressive text cleaner in `file-parser.ts` to preserve short technical skills (*JS, C, R, AI, ML, UI, UX, AWS, SQL, Git*).
