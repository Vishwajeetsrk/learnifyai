...
[...allSrcFiles, ...allPublicFiles].forEach(f => {
  ...
  if (stat.size > 1024 * 1024) {
    largeFiles.push({ file: f.replace(ROOT.replace(/\\/g, '/') + '/', ''), size: (stat.size / 1024 / 1024).toFixed(2) + ' MB' });
  }
});
...
const fileUsage = {};
allSrcFiles.forEach(f => { fileUsage[f] = 0; });
...
deps.forEach(dep => {
  ...
  try {
    ...
    execSync(`git grep -E "from ['\\"]${dep}['\\"]|require\\(['\\"]${dep}['\\"]\\)"`, { cwd: ROOT, stdio: 'ignore' });
  } catch (e) {
