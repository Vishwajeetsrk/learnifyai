...
(async () => {
  ...
  page.on('response', async (res) => {
    const url = res.url();
    if (url.includes('/user_roles') || url.includes('/profiles') || url.includes('/auth/v1/token')) {
  ...
  try {
    ...
    console.log('heading count', await adminHeading.count(), 'visible', await adminHeading.isVisible().catch(() => false));
