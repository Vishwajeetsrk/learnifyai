...
(async () => {
  ...
  try {
    ...
    await Promise.all([
      page.waitForResponse((res) => res.url().includes('/auth/v1/token') && res.status() === 200, { timeout: 30000 }),
    ...
    for (const tab of tabs) {
      ...
      console.log('tab', tab, 'count', await trigger.count(), 'visible', await trigger.isVisible().catch(() => false));
      if (await trigger.isVisible().catch(() => false)) {
        await trigger.click({ force: true }).catch((e) => console.error('click tab error', tab, e.message));
    ...
    console.log('save button count', await saveBtn.count(), 'visible', await saveBtn.isVisible().catch(() => false));
  } catch (error) {
