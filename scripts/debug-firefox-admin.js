...
(async () => {
  ...
  try {
    ...
    if (await emailInput.isVisible({ timeout: 10000 }).catch(() => false)) {
      ...
      const toast = await page.locator('text=/error|failed|invalid|incorrect|toast/i').allTextContents().catch(() => []);
      console.log('potential toast texts', toast);
      await page.waitForURL(/\/dashboard|\/admin\/content|\/login|\//, { timeout: 60000 }).catch(() => {});
      console.log('after sign in url', page.url());
    } else {
      console.log('email input not visible');
    }

    await page.goto(`${base}/admin/content`, { waitUntil: 'networkidle', timeout: 60000 }).catch(() => {});
