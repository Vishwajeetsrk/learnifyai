...
type SectionKey = keyof typeof STELLAR_SECTIONS;

export default function StellarSectionPage({ sectionKey }: { sectionKey: SectionKey }) {
  const section = STELLAR_SECTIONS[sectionKey];
  ...
  return (
    ...
    >
      <div className="mx-auto grid max-w-7xl gap-10 md:grid-cols-2 md:items-center md:gap-16">
        <div>
          <p className="animate-fade-in-up text-xs font-medium uppercase tracking-[0.3em] text-violet-300/80">
            {section.eyebrow}
          </p>
          <h2 className="animate-fade-in-up mt-3 font-display text-3xl font-semibold tracking-tight text-white md:text-4xl">
            {section.title}
          </h2>
          <p className="animate-fade-in-up mt-4 text-base leading-relaxed text-white/55 md:text-lg">
            {section.body}
          </p>
          <ul className="animate-fade-in-up mt-6 space-y-2">
        ...
        <div className="animate-fade-in-up liquid-glass flex aspect-[4/3] flex-col items-center justify-center rounded-3xl p-8">
          <Icon className="h-12 w-12 text-violet-300/80" strokeWidth={1.25} />
          <p className="mt-4 text-center text-sm text-white/45">{section.eyebrow} preview panel</p>
        </div>
      </div>
    </section>
  );
