...
test.describe('Authentication Flows', () => {
  test('should load the login page', async ({ page }) => {
    ...
    await expect(page.getByRole('heading', { name: /login/i, level: 1 })).toBeVisible({ timeout: 10000 }).catch(() => null);
  ...
  test('should load the signup page', async ({ page }) => {
    ...
    await expect(page.getByRole('heading', { name: /sign up/i, level: 1 })).toBeVisible({ timeout: 10000 }).catch(() => null);
