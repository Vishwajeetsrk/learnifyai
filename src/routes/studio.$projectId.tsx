...
import { ArrowLeft, Play, Pause, ChevronRight, ChevronLeft, Code2, Layout, Maximize2, Monitor, Box, BookOpen, Flame, Star, CheckCircle2 } from "lucide-react";
    ...
    <div className="h-screen w-full bg-background flex flex-col font-sans overflow-hidden relative">
      ...
      <header className="h-14 border-b border-border bg-card flex items-center justify-between px-4 shrink-0 z-10">
        <div className="flex items-center gap-4">
          <Link to="/course/$projectId" params={{ projectId: project.id }} className="p-2 -ml-2 rounded-lg text-muted-foreground hover:text-foreground hover:bg-accent transition-colors">
        ...
        <aside className={cn(
          ...
          {sidebarMode === "curriculum" ? (
            <>
              ...
              <div className="p-4 border-b border-border space-y-4">
                 <div className="flex items-center justify-between">
                    ...
                    <span className="text-xs font-semibold text-muted-foreground">MODULE {activeStep + 1}</span>
                    <button 
                      onClick={() => setActiveStep(prev => Math.min(modules.length - 1, prev + 1))}
                      disabled={activeStep === modules.length - 1 || (!isStepCompleted && currentModule?.quiz)}
                      className="p-1.5 rounded-lg bg-accent text-foreground disabled:opacity-50 disabled:cursor-not-allowed hover:bg-accent/80 transition"
                      title={(!isStepCompleted && currentModule?.quiz) ? "Complete the quiz to unlock the next step!" : "Next Step"}
                    >
                 ...
                 <div className="h-1.5 w-full bg-accent rounded-full overflow-hidden">
                    <motion.div 
                      ...
                      initial={{ width: 0 }}
                      animate={{ width: `${((activeStep + 1) / Math.max(1, modules.length)) * 100}%` }}
                    />
                 </div>
              </div>
                 ...
                 {currentModule ? (
                   <AnimatePresence mode="wait">
                     <motion.div
                       ...
                       className="space-y-4"
                     >
                       <h2 className="text-lg font-display font-semibold leading-snug">{currentModule.title}</h2>
                       <p className="text-sm text-muted-foreground leading-relaxed">{currentModule.text}</p>
                       ...
                       {currentModule.voice_script && (
                         <div className="p-4 rounded-xl border border-primary/20 bg-primary/5 flex items-start gap-4 mt-6">
                           ...
                           <div className="space-y-1">
                             <p className="text-xs font-semibold text-primary">Listen to the Breakdown</p>
                       ...
                       {currentModule.quiz && (
                         <div className="mt-8 bg-card border border-border rounded-xl p-5 shadow-sm">
                           ...
                           <p className="text-sm text-foreground mb-4">{currentModule.quiz.question}</p>
                           <div className="space-y-2">
                             {currentModule.quiz.options.map((option: string, index: number) => {
                               ...
                               return (
                                 <button
                                   key={index}
                                   onClick={() => handleQuizAnswer(index, currentModule.quiz.correct_index)}
                           ...
                           {selectedAnswer !== null && (
                             ...
                             >
                               <span className="font-semibold text-foreground">Explanation:</span> {currentModule.quiz.explanation}
                             </motion.div>
                           )}
                         </div>
                       )}
                   ...
                   <div className="text-sm text-muted-foreground italic">No content available for this step.</div>
                 )}
              </div>
            </>
          ) : (
            <div className="flex-1 overflow-y-auto bg-black/40">
               {/* @ts-ignore */}
               <ArchitectureGraph nodes={project?.architecture_nodes || []} />
            </div>
          )}
...
function ArchitectureGraph({ nodes }: { nodes: any[] }) {
  if (!nodes || nodes.length === 0) return <div className="p-6 text-sm text-muted-foreground text-center">No architecture data available for this project.</div>;
      ...
      >
        <div className="flex items-center gap-2 py-2" style={{ paddingLeft: depth * 16 }}>
           ...
           <div className="h-6 w-6 rounded-md bg-accent/80 flex items-center justify-center shrink-0 border border-border/50 text-muted-foreground shadow-sm z-10">
             {node.type === "container" ? <Layout className="h-3 w-3" /> :
              node.type === "media" ? <Play className="h-3 w-3" /> :
              node.type === "animation" ? <Code2 className="h-3 w-3" /> :
        ...
        {node.children && node.children.length > 0 && (
          <div className="flex flex-col relative before:absolute before:left-[11px] before:top-0 before:bottom-0 before:w-[1px] before:bg-border/60" style={{ marginLeft: depth * 16 }}>
  ...
  return (
    <div className="p-4 overflow-x-hidden pb-12">
      <div className="mb-4 text-xs font-semibold text-muted-foreground tracking-widest uppercase">DOM Blueprint</div>
