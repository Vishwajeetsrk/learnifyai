...
function BillingOSPage() {
  ...
  const creditsQuery = useQuery({
    ...
    queryFn: async () => {
      ...
      if (creditSearch) {
        query = query.or(`user_id.ilike.%${creditSearch}%`);
      }
  ...
  async function handleExport() {
    ...
    try {
      ...
      if (exportFormat === "json") {
        ...
        const csv = [
          headers.join(","),
          ...res.rows.map((r: any) => headers.map((h) => JSON.stringify(r[h] ?? "")).join(",")),
        ].join("\n");
  ...
  const overviewCards = overviewData
    ? [
        {
          label: "Total Revenue",
          value: inr(overviewData.total_revenue),
          icon: IndianRupee,
        ...
        },
        {
          ...
          value: overviewData.active_subscribers.toLocaleString(),
          icon: Users,
          color: "text-cyan-500",
        ...
        },
        {
          ...
          value: `${overviewData.invoices_this_month_count} · ${inr(overviewData.invoices_this_month_total)}`,
          icon: Receipt,
        ...
        },
        {
          ...
          value: `${overviewData.payment_success_rate}%`,
          icon: Activity,
          color: "text-emerald-500",
          bgColor: "bg-emerald-500/10",
        },
        {
          label: "Refund Count",
          value: overviewData.refund_count.toLocaleString(),
          icon: AlertTriangle,
          color: "text-red-500",
        ...
        },
        {
          ...
          value: overviewData.credits_remaining.toLocaleString(),
          icon: Wallet,
        ...
        {
          label: "Credits Used",
          value: overviewData.credits_used.toLocaleString(),
          icon: Layers,
          color: "text-zinc-500",
                ...
                <DialogContent className="sm:max-w-lg">
                  ...
                  <DialogFooter>
                    ...
                    <Button
                      ...
                      disabled={creatingInvoice || !manualUserId || !manualAmount}
                    >
                      {creatingInvoice && <Loader2 className="h-4 w-4 mr-1 animate-spin" />}Create
                      Invoice
                    </Button>
                  </DialogFooter>
                </DialogContent>
                    ...
                    {invoices.data.invoices.map((inv: any) => (
                      <TableRow key={inv.id}>
                        ...
                        <TableCell className="font-medium">
                          {inr(Number(inv.total_inr) || 0)}
                        </TableCell>
                        <TableCell>
                          <Badge
                            className={cn(
                              "text-xs border",
                              STATUS_BADGE[inv.status] || "bg-zinc-500/10 text-zinc-500",
                            )}
                          >
                            {inv.status}
                          </Badge>
            ...
            <div className="flex items-center justify-between text-sm text-muted-foreground">
              ...
              <div className="flex gap-2">
                ...
                <Button
                  variant="outline"
                  size="sm"
                  disabled={(invoices.data?.invoices?.length || 0) < 20}
                  onClick={() => setInvoicePage((p) => p + 1)}
                >
                  Next
                </Button>
            ...
            <div className="flex items-center justify-between text-sm text-muted-foreground">
              ...
              <div className="flex gap-2">
                ...
                <Button
                  variant="outline"
                  size="sm"
                  disabled={(payments.data?.logs?.length || 0) < 20}
                  onClick={() => setPaymentPage((p) => p + 1)}
                >
                  Next
                </Button>
                          ...
                          <TableRow key={sub.id}>
                            ...
                            <TableCell>
                              <Badge
                                className={cn(
                                  "text-xs border",
                                  STATUS_BADGE[sub.status] || "bg-zinc-500/10 text-zinc-500",
                                )}
                              >
                                {sub.status}
                              </Badge>
            ...
            {overviewData && (
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                ...
                <StatCard
                  label="Total Credits"
                  value={(
                    overviewData.credits_remaining + overviewData.credits_used
                  ).toLocaleString()}
                  icon={CreditCard}
                  color="text-violet-500"
                  bgColor="bg-violet-500/10"
                ...
                <StatCard
                  ...
                  value={
                    overviewData.active_subscribers > 0
                      ? Math.round(
                          (overviewData.credits_remaining + overviewData.credits_used) /
                            Math.max(overviewData.active_subscribers, 1),
                        ).toLocaleString()
                      : "—"
                  }
                  icon={Activity}
                  ...
                  <TableBody>
                    {creditsQuery.data.map((credit: any) => (
                      <TableRow key={credit.id}>
                        <TableCell>
                          <div className="text-sm">
                            {credit.user?.full_name ||
                              credit.user?.email ||
                              credit.user_id?.slice(0, 8)}
                          </div>
                        </TableCell>
                        <TableCell className="font-medium text-emerald-500">
                          {credit.credits_remaining?.toLocaleString() || 0}
                        </TableCell>
                        <TableCell className="font-medium text-blue-500">
                          {credit.credits_used?.toLocaleString() || 0}
                        </TableCell>
                        <TableCell className="font-medium">
                          {(
                            Number(credit.credits_remaining) + Number(credit.credits_used)
                          ).toLocaleString()}
                        </TableCell>
                        <TableCell className="text-xs text-muted-foreground">
                          {credit.updated_at
                ...
                <DialogContent className="sm:max-w-lg">
                  ...
                  <DialogFooter>
                    ...
                    <Button
                      ...
                      disabled={processingRefund || !refundInvoiceId || !refundAmount}
                    >
                      {processingRefund && <Loader2 className="h-4 w-4 mr-1 animate-spin" />}Process
                      Refund
                    </Button>
                  </DialogFooter>
                </DialogContent>
                ...
                <Table>
                  ...
                  <TableBody>
                    {refundsData.data.refunds.map((ref: any) => (
                      <TableRow key={ref.id}>
                        <TableCell className="font-mono text-xs">
                          {ref.invoice?.invoice_number || ref.invoice_id?.slice(0, 8)}
                        </TableCell>
                        <TableCell>
                          <div className="text-sm">
                            {ref.user?.full_name || ref.user?.email || ref.user_id?.slice(0, 8)}
                          </div>
                        </TableCell>
                        <TableCell className="font-medium">
                          {ref.amount_inr ? inr(Number(ref.amount_inr)) : "—"}
                        </TableCell>
                        ...
                        <TableCell>
                          <Badge
                            className={cn(
                              "text-xs border",
                              STATUS_BADGE[ref.status] || "bg-zinc-500/10 text-zinc-500",
                            )}
                          >
                            {ref.status}
                          </Badge>
                        ...
                        <TableCell className="text-xs text-muted-foreground">
                          {ref.created_at ? format(new Date(ref.created_at), "MMM d, yyyy") : "—"}
                        </TableCell>
                      </TableRow>
                    ))}
                  </TableBody>
            ...
            <div className="flex items-center justify-between text-sm text-muted-foreground">
              ...
              <div className="flex gap-2">
                ...
                <Button
                  variant="outline"
                  size="sm"
                  disabled={(refundsData.data?.refunds?.length || 0) < 20}
                  onClick={() => setRefundPage((p) => p + 1)}
                >
                  Next
                </Button>
              ...
              <div className="rounded-xl border bg-card p-6 space-y-6">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground mb-4">
                    India GST tax settings for invoice generation.
                  </p>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                  <div className="space-y-1.5">
                    <Label>GSTIN</Label>
                    <Input
                      defaultValue={billingSettings.data?.gstin || ""}
                      onBlur={(e) => handleSaveSetting("gstin", e.target.value)}
                  ...
                  <div className="space-y-1.5">
                    ...
                    <Input
                      defaultValue={billingSettings.data?.hsn_code || ""}
                      onBlur={(e) => handleSaveSetting("hsn_code", e.target.value)}
                      placeholder="8471"
                    />
                  </div>
                  <div className="space-y-1.5">
                    ...
                    <Input
                      defaultValue={billingSettings.data?.sac_code || ""}
                      onBlur={(e) => handleSaveSetting("sac_code", e.target.value)}
                      placeholder="9983"
                    />
                  </div>
                  <div className="space-y-1.5">
                    ...
                    <Input
                      defaultValue={billingSettings.data?.cgst_rate || ""}
                      onBlur={(e) => handleSaveSetting("cgst_rate", e.target.value)}
                      placeholder="9"
                    />
                  </div>
                  <div className="space-y-1.5">
                    ...
                    <Input
                      defaultValue={billingSettings.data?.sgst_rate || ""}
                      onBlur={(e) => handleSaveSetting("sgst_rate", e.target.value)}
                      placeholder="9"
                    />
                  </div>
                  <div className="space-y-1.5">
                    ...
                    <Input
                      defaultValue={billingSettings.data?.igst_rate || ""}
                      onBlur={(e) => handleSaveSetting("igst_rate", e.target.value)}
                      placeholder="18"
                    />
                  </div>
                  <div className="flex items-center gap-3">
                    <Switch
                      defaultChecked={billingSettings.data?.tds_enabled === "true"}
                      onCheckedChange={(v) => handleSaveSetting("tds_enabled", String(v))}
                    />
                  ...
                  <div className="space-y-1.5">
                    ...
                    <Input
                      defaultValue={billingSettings.data?.tds_rate || ""}
                      onBlur={(e) => handleSaveSetting("tds_rate", e.target.value)}
                      placeholder="10"
                    />
                  </div>
                </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="space-y-1.5">
                      ...
                      <Input
                        ...
                        defaultValue={billingSettings.data?.company_name || ""}
                        placeholder="Learnify AI"
                      />
                    </div>
                    <div className="space-y-1.5">
                      <Label>Legal Name</Label>
                      <Input
                        id="setting-legal_name"
                        defaultValue={billingSettings.data?.legal_name || ""}
                        placeholder="Learnify AI Pvt. Ltd."
                      />
                    </div>
                    <div className="space-y-1.5">
                      <Label>Logo URL</Label>
                      <Input
                        id="setting-company_logo_url"
                        defaultValue={billingSettings.data?.company_logo_url || ""}
                        placeholder="https://..."
                      />
                    </div>
                    <div className="space-y-1.5">
                      ...
                      <Input
                        id="setting-brand_color"
                        defaultValue={billingSettings.data?.brand_color || ""}
                        placeholder="#6366f1"
                      />
                    </div>
                  </div>
                ...
                <div className="rounded-xl border bg-card p-6 space-y-4">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">
                        Defaults for generated invoices.
                      </p>
                    </div>
                    <Button
                      size="sm"
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="space-y-1.5">
                      ...
                      <Input
                        id="setting-invoice_prefix"
                        defaultValue={billingSettings.data?.invoice_prefix || ""}
                        placeholder="INV"
                      />
                    </div>
                    <div className="space-y-1.5">
                      ...
                      <Input
                        id="setting-invoice_currency"
                        defaultValue={billingSettings.data?.invoice_currency || ""}
                        placeholder="INR"
                      />
                    </div>
                    <div className="space-y-1.5 md:col-span-2">
                      ...
                      <Input
                        id="setting-invoice_footer"
                        defaultValue={billingSettings.data?.invoice_footer || ""}
                    ...
                    <div className="space-y-1.5 md:col-span-2">
                      ...
                      <Input
                        id="setting-invoice_terms"
                        defaultValue={billingSettings.data?.invoice_terms || ""}
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div className="space-y-1.5">
                      ...
                      <Input
                        ...
                        defaultValue={billingSettings.data?.support_phone || ""}
                    ...
                    <div className="space-y-1.5 md:col-span-2">
                      <Label>Address</Label>
                      <Input
                        id="setting-support_address"
                        defaultValue={billingSettings.data?.support_address || ""}
            ...
            <TableBody>
              {coupons.map((c: any) => (
                <TableRow key={c.id}>
                  ...
                  <TableCell>
                    {c.valid_until ? format(new Date(c.valid_until), "MMM d, yyyy") : "Never"}
                  </TableCell>
                  <TableCell>
                ...
                <div className="space-y-1.5">
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      setEditing({
                        ...editing,
                        discount_percent: e.target.value ? Number(e.target.value) : undefined,
                        discount_amount_inr: undefined,
                      })
                    }
                    placeholder="10"
                  />
                ...
                <div className="space-y-1.5">
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      setEditing({
                        ...editing,
                        discount_amount_inr: e.target.value ? Number(e.target.value) : undefined,
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div className="space-y-1.5">
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      setEditing({ ...editing, valid_until: e.target.value || undefined })
                    }
                  />
                </div>
              </div>
...
function SubscriptionActions({ sub }: { sub: any }) {
  ...
  const handleAction = async (
    ...
    const label = { activate: "Activate", cancel: "Cancel", pause: "Pause", extend: "Extend" }[
      action
    ];
