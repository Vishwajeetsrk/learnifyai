...
export const Route = createFileRoute("/_authenticated/admin/visual-learning")({
...
function AdminVisualLearningPage() {
  ...
  const filteredConcepts = search
    ? concepts.filter(
        (c: any) =>
          c.lesson_id?.toLowerCase().includes(search.toLowerCase()) ||
          c.course_id?.toLowerCase().includes(search.toLowerCase()),
      )
    : concepts;
  const filteredExplanations = search
    ? explanations.filter(
        (e: any) =>
          e.lesson_id?.toLowerCase().includes(search.toLowerCase()) ||
        ...
        <Tabs value={tab} onValueChange={setTab}>
          <TabsList>
            <TabsTrigger value="concepts" className="text-xs gap-1.5">
              <Brain className="h-3.5 w-3.5" /> Concept Graphs ({concepts.length})
            </TabsTrigger>
            <TabsTrigger value="explanations" className="text-xs gap-1.5">
              <Lightbulb className="h-3.5 w-3.5" /> Explain Like I'm 12 ({explanations.length})
            </TabsTrigger>
          </TabsList>
          ...
          <div className="relative max-w-xs mt-3">
            ...
            <Input
              ...
              placeholder="Search by lesson/course ID..."
          ...
          <TabsContent value="concepts" className="mt-3 space-y-3">
            {conceptsQuery.isLoading ? (
              ...
              <div className="p-8 text-center text-sm text-muted-foreground">
                No concept graphs found. They are generated by AI when viewing a lesson.
              </div>
            ) : (
                      ...
                      <div className="min-w-0 flex-1">
                        <div className="flex items-center gap-2 flex-wrap">
                          <Badge variant="secondary" className="text-[9px]">
                            Lesson: {c.lesson_id?.slice(0, 8)}...
                          </Badge>
                          {c.course_id && (
                            <Badge variant="outline" className="text-[9px]">
                              Course: {c.course_id?.slice(0, 8)}...
                            </Badge>
                          )}
                        </div>
                        <p className="text-xs text-muted-foreground mt-1">
                          Nodes: {c.nodes?.length || 0} · Edges: {c.edges?.length || 0}
                        </p>
                        <p className="text-[10px] text-muted-foreground mt-0.5">
                          Generated:{" "}
                          {c.generated_at ? new Date(c.generated_at).toLocaleString() : "N/A"}
                        </p>
                      </div>
                    ...
                    {selectedItem?.id === c.id && (
                      <div className="mt-3 p-3 rounded-lg bg-muted/50 max-h-60 overflow-auto">
                        <p className="text-[10px] font-mono text-muted-foreground whitespace-pre-wrap">
                          {JSON.stringify({ nodes: c.nodes, edges: c.edges }, null, 2).slice(
                            0,
                            2000,
                          )}
                        </p>
          ...
          <TabsContent value="explanations" className="mt-3 space-y-3">
            {explanationsQuery.isLoading ? (
              ...
              <div className="p-8 text-center text-sm text-muted-foreground">
                No explanations cached. They are generated by AI when using Explain Like I'm 12.
              </div>
            ) : (
                    ...
                    <div className="flex items-start justify-between gap-3">
                      ...
                      <div className="flex items-center gap-1 shrink-0">
                        <Button
                          ...
                          size="sm"
                          className="h-7 w-7 p-0"
                          onClick={() => setSelectedItem(selectedItem?.id === e.id ? null : e)}
                        >
                          <Eye className="h-3 w-3" />
                        </Button>
                        <Button
