...
import { Bot, Send, Loader2, Sparkles, Trash2, Play, Terminal, ArrowRight, MessageSquare, Brain } from "lucide-react";
...
function BotAvatar({ className }: { className?: string }) {
  return (
    <div className={cn("h-9 w-9 rounded-full bg-gradient-to-br from-violet-500 to-fuchsia-500 flex items-center justify-center shrink-0 shadow-sm ring-2 ring-background", className)}>
...
function SupportPage() {
  ...
  const [messages, setMessages] = useState<Array<{ role: "user" | "assistant"; content: string; steps?: any[] }>>([]);
    ...
    <AppShell>
      <div className="max-w-4xl mx-auto px-4 md:px-10 py-8">
        <div className="mb-6">
          ...
          <h1 className="mt-1 text-3xl font-display font-semibold tracking-tight">Learnify AI Assistant</h1>
            ...
            {messages.length === 0 ? (
              <div className="text-center py-12 space-y-5">
                ...
                <p className="text-sm text-muted-foreground max-w-md mx-auto">
                  I'm your AI support assistant. Ask me about any feature, tool, or setting on Learnify.
                </p>
                ...
                <div className={cn(
                  ...
                  >
                    <ReactMarkdown
                      ...
                      components={{
                        pre: ({ children }) => (
                          <pre className="bg-muted rounded-lg p-3 my-2 overflow-x-auto text-xs">{children}</pre>
                        ),
                        ...
                        a: ({ href, children }) => (
                          <a href={href} className="text-primary underline hover:text-primary/80 font-medium">{children}</a>
                        ),
                      }}
                    >
                      {m.content}
                    ...
                    <div key={si} className="text-xs">
                      {step.type === "tool_call" && step.name === "execute_code" && (
                        <details className="rounded-lg border border-primary/20 bg-primary/5 p-2.5 space-y-1.5">
                          ...
                          {step.arguments?.code && (
                            <pre className="text-[10px] bg-black/80 text-green-400 rounded p-2 overflow-x-auto">{step.arguments.code}</pre>
                          )}
                          {step.result && (
                            <pre className="text-[10px] bg-black/90 text-white rounded p-2 overflow-x-auto">{step.result}</pre>
                          )}
                        </details>
                      )}
