  ...
  const { data: submissions } = useQuery({
    ...
    queryFn: async () => {
      ...
      const solved = new Set(
        (data ?? []).filter((s: any) => s.passed).map((s: any) => s.challenge_id),
      );
      return solved;
    },
  });
        ...
        <div className="flex items-center gap-3 mb-2">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              Solve problems, earn points, and sharpen your skills.
            </p>
          </div>
        </div>
            ...
            {challenges.map((c: any) => {
              ...
              return (
                <Link
                  ...
                  search={{ challenge: c.slug } as any}
                  className="flex items-center gap-4 rounded-xl border bg-card p-4 hover:shadow-md transition group"
                >
                  <div
                    className={cn(
                      "w-10 h-10 rounded-lg grid place-items-center shrink-0",
                      solved ? "bg-green-500/10" : "bg-muted",
                    )}
                  >
                    {solved ? (
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-3 mt-1 text-[10px] text-muted-foreground">
                      ...
                      <span className="flex items-center gap-1">
                        <Clock className="h-3 w-3" /> {c.points} pts
                      </span>
                    </div>
                  </div>
                  <Badge className={cn("text-[10px] capitalize", diffColor(c.difficulty))}>
                    {c.difficulty}
                  </Badge>
