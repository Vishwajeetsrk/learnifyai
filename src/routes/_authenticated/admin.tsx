...
function AdminOverview() {
  ...
  const handleExport = async () => {
    ...
    dailyWs.addRow([]);
    dailyWs.addRow(["TOTAL", dailySeries.reduce((s, d) => s + d.revenue, 0)]);
    ...
    weeklyWs.addRow([]);
    weeklyWs.addRow(["TOTAL", weeklySeries.reduce((s, w) => s + w.revenue, 0)]);
    ...
    styleSheet(txWs, [38, 38, 14, 10, 12, 35, 20], "FF2E75B6");
    ...
    styleSheet(usersWs, [38, 22, 30, 28, 12, 12, 14, 20], "FF548235");

    // ─── 6. AI COST BREAKDOWN ───
    const aiCosts = aiCostQuery.data ?? [];
    if (aiCosts.length > 0) {
      const aiWs = wb.addWorksheet("AI Costs");
      aiWs.addRow(["Model", "Cost (USD)", "Cost (INR)", "Tokens", "Date"]);
      aiCosts.forEach((a) =>
        aiWs.addRow([
          a.model ?? "unknown",
      ...
      aiWs.addRow([]);
      aiWs.addRow(["TOTAL", totalUsd.toFixed(4), totalInr.toFixed(2), totalTokens, ""]);
      styleSheet(aiWs, [25, 14, 14, 14, 20], "FFBF8F00");
    }
