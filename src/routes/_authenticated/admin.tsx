...
type RangePreset = "7d" | "month" | "90d" | "custom";
...
function AdminOverview() {
  ...
  const [from, setFrom] = useState<Date>(subDays(today, 6));
  ...
  // Creator/Coach applications queue
  ...
  const handleExport = async () => {
    ...
    // ─── DAILY REVENUE ───
    const dailyWs = wb.addWorksheet("Daily Revenue");
    dailyWs.addRow(["Date", "Revenue (₹)"]);
    dailySeries.forEach((d) => dailyWs.addRow([d.date, d.revenue]));
    dailyWs.addRow([]);
    const dailyTotal = dailySeries.reduce((s, d) => s + d.revenue, 0);
    dailyWs.addRow(["TOTAL", dailyTotal]);
    const dtRow = dailyWs.lastRow!;
    dtRow.font = { bold: true, size: 12 };
    dtRow.fill = { type: "pattern", pattern: "solid", fgColor: { argb: "FFE8F0FE" } };
    styleSheet(dailyWs, [18, 18], "FF4472C4");
    // Data bars on revenue column
    if (dailySeries.length > 0) {
      const maxRev = Math.max(...dailySeries.map((d) => d.revenue), 1);
    ...
    // ─── WEEKLY REVENUE ───
    const weeklyWs = wb.addWorksheet("Weekly Revenue");
    weeklyWs.addRow(["Week Starting", "Revenue (₹)"]);
    weeklySeries.forEach((w) => weeklyWs.addRow([w.week, w.revenue]));
    weeklyWs.addRow([]);
    const weeklyTotal = weeklySeries.reduce((s, w) => s + w.revenue, 0);
    weeklyWs.addRow(["TOTAL", weeklyTotal]);
    const wtRow = weeklyWs.lastRow!;
    wtRow.font = { bold: true, size: 12 };
    wtRow.fill = { type: "pattern", pattern: "solid", fgColor: { argb: "FFE8F0FE" } };
    styleSheet(weeklyWs, [20, 18], "FF4472C4");

    // ─── REVENUE BREAKDOWN (Pie-style table) ───
    ...
    if (aiCosts.length > 0) {
      const aiWs = wb.addWorksheet("AI Costs");
    ...
    a.href = url;
    a.download = `Learnify-Report_${fmtDate(from)}_to_${fmtDate(to)}.xlsx`;
    a.click();
            ...
            <div className="flex flex-wrap items-center gap-2 w-full">
              ...
              <div className="grid grid-cols-3 sm:flex sm:flex-wrap gap-2 w-full sm:w-auto">
                {(["7d", "month", "90d"] as const).map((p) => (
                  ...
                  >
                    {p === "7d" ? "7 days" : p === "month" ? "This month" : "90 days"}
                  </Button>
                ))}
              </div>
            </div>
        ...
        <div className="mt-6 rounded-2xl border bg-card shadow-card overflow-hidden">
          <div className="px-6 py-4 border-b">
            <h2 className="font-display text-lg font-semibold flex items-center gap-2">
              <Wallet className="h-4 w-4 text-primary" /> Wallet Top-ups
            </h2>
            <p className="text-xs text-muted-foreground">
              All top-up transactions processed via Cashfree
            </p>
          </div>
          {txQuery.isLoading ? (
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead className="bg-muted/40 text-xs uppercase tracking-wide text-muted-foreground">
                  <tr>
                    <th className="text-left px-4 md:px-6 py-3 font-medium">When</th>
        ...
        <div className="flex items-center gap-3 min-w-0 flex-1" onClick={() => setDetailApp(app)}>
          <div className="h-9 w-9 rounded-full bg-muted overflow-hidden shrink-0 ring-2 ring-border">
            {getAvatar(app) ? (
              ...
              <div className="h-full w-full flex items-center justify-center text-sm font-bold text-muted-foreground bg-gradient-to-br from-primary/10 to-primary/5">
                {(getProfile(app)?.charAt(0) || "?").toUpperCase()}
              </div>
            )}
          </div>
