        ...
        {tab === "purchases" && (
          <>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              ...
              <StatCard
                label="Unique Users"
                value={String(new Set(purchases.map((p: any) => p.userId)).size)}
                color="text-purple-500"
              />
              <StatCard
                label="Avg Cost"
                value={
                  purchases.length > 0 ? `${Math.round(totalRevenue / purchases.length)} XP` : "—"
                }
                          ...
                          <td className="px-3 sm:px-4 py-3">
                            <div className="flex items-center gap-2">
                              ...
                              <div className="min-w-0">
                                ...
                                <p className="text-[10px] text-muted-foreground truncate max-w-[100px] sm:max-w-none hidden sm:block">
                                  {p.user?.email || ""}
                                </p>
                              </div>
                            </div>
          ...
          <>
            <div className="flex items-center justify-between gap-3">
              <p className="text-xs text-muted-foreground">
                {items.length} items · Click to edit or delete
              </p>
            ...
            <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
              {itemsQuery.isLoading ? (
                ...
                items.map((item: any) => (
                  <Card
                    key={item.id}
                    className={cn("relative overflow-hidden", !item.enabled && "opacity-60")}
                  >
                    {item.image_url && (
                      <img
                        src={item.image_url}
                          ...
                          {item.image_url ? (
                            ...
                            <div
                              className={cn(
                                ...
                                item.color || "from-primary to-primary/60",
                              )}
                            >
                              {item.is_prime ? (
                      ...
                      <div className="flex flex-wrap gap-1 mt-2">
                        {item.auto_claim && (
                          <Badge variant="secondary" className="text-[9px]">
                            <Check className="h-2.5 w-2.5 mr-0.5" /> Auto-claim
                          </Badge>
                        )}
                        {item.stock != null && (
                          <Badge variant="outline" className="text-[9px]">
                            <Package className="h-2.5 w-2.5 mr-0.5" /> Stock: {item.stock}
                          </Badge>
                        )}
                        {!item.enabled && (
                      ...
                      {item.tags?.length > 0 && (
                        <div className="flex flex-wrap gap-1 mt-1.5">
                          {item.tags.map((tag: string) => (
                            <Badge key={tag} variant="default" className="text-[8px] h-4">
                              {tag}
                            </Badge>
                          ))}
                        </div>
          ...
          <DialogContent className="sm:max-w-md max-h-[90vh] overflow-y-auto">
            <DialogHeader>
              <DialogTitle className="flex items-center gap-2">
                <ShoppingCart className="h-4 w-4" /> {editingItem ? "Edit Item" : "Add Store Item"}
              </DialogTitle>
            </DialogHeader>
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-3">
                ...
                <div className="space-y-1.5 col-span-2">
                  ...
                  <Input
                    ...
                    onChange={(e) => setItemForm((f) => ({ ...f, perks: e.target.value }))}
                    placeholder="resume_premium, gold_frame"
                  />
                </div>
                <div className="space-y-1.5 col-span-2">
                ...
                <div className="space-y-1.5">
                  <Label className="flex items-center gap-1">
                    <Package className="h-3 w-3" /> Stock (optional)
                  </Label>
                  <Input
                    type="number"
                    min={0}
                ...
                <div className="space-y-1.5">
                  ...
                  <Input
                    ...
                    placeholder="popular, new, limited"
                  />
                </div>
              </div>
              <div className="flex flex-col gap-3 rounded-lg border p-3">
                ...
                {itemForm.is_prime && (
                  <div className="flex items-center gap-3">
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        setItemForm((f) => ({ ...f, prime_price: Number(e.target.value) }))
                      }
                      className="h-8 w-28"
                    />
                  </div>
                ...
                <div className="flex items-center justify-between">
                  ...
                  <Switch
                    checked={itemForm.auto_claim}
                    onCheckedChange={(v) => setItemForm((f) => ({ ...f, auto_claim: v }))}
                  />
                </div>
                <div className="flex items-center justify-between">
