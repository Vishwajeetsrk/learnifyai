...
import { Plus, LayoutGrid, Calendar, FolderKanban, MoreVertical, Search, CheckCircle2, Circle, Clock, Trash2 } from "lucide-react";
...
import { getMyWorkspace, listProjects, createProject, getProjectTasks, createTask, updateTaskStatus, deleteTask } from "@/lib/workspace.functions";
    ...
    <AppShell>
            ...
            {projects.map((p: any) => (
              <button
                ...
                className={`w-full flex items-center gap-2 px-3 py-2 text-sm rounded-md transition-colors ${
                  activeProjectId === p.id ? "bg-primary/10 text-primary font-medium" : "hover:bg-slate-100 text-slate-700"
                }`}
              >
        ...
        <div className="flex-1 flex flex-col min-w-0 bg-slate-50 overflow-hidden">
          {activeProjectId ? (
              ...
              <div className="h-14 border-b bg-white flex items-center justify-between px-6 shrink-0">
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex gap-1">
                    ...
                    <Button variant="ghost" size="sm" className="h-8 text-muted-foreground"><Calendar className="h-4 w-4 mr-2" /> Calendar</Button>
                  </div>
                </div>
                <div className="flex items-center gap-2">
                  <div className="relative">
                    <Search className="h-4 w-4 absolute left-3 top-1/2 -translate-y-1/2 text-muted-foreground" />
                    <Input className="h-8 w-64 pl-9 text-sm bg-slate-50" placeholder="Search tasks..." />
                  </div>
                  <Button size="sm" onClick={() => setNewTaskOpen(true)}><Plus className="h-4 w-4 mr-2" /> Add Task</Button>
                </div>
              </div>
                            ...
                            <div className="flex items-center justify-between mt-3">
                              <div className="flex -space-x-1">
                                {task.assignee?.avatar_url ? (
                                  <img src={task.assignee.avatar_url} className="h-6 w-6 rounded-full ring-2 ring-white" alt="Assignee" />
                                ) : (
            ...
            <div className="flex-1 flex flex-col items-center justify-center text-center p-10 text-muted-foreground">
              ...
              <p className="max-w-sm mt-2">Select a project from the sidebar or create a new one to start organizing your work.</p>
              <Button className="mt-6" onClick={() => setNewProjOpen(true)}><Plus className="h-4 w-4 mr-2" /> Create Project</Button>
            </div>
          )}
        </div>
      </div>
      ...
      <Dialog open={newProjOpen} onOpenChange={setNewProjOpen}>
        <DialogContent>
          ...
          <div className="py-4">
            <Input autoFocus value={projName} onChange={e => setProjName(e.target.value)} placeholder="Project Name e.g. 'Course Launch'" />
          </div>
          <DialogFooter>
            <Button variant="outline" onClick={() => setNewProjOpen(false)}>Cancel</Button>
      ...
      <Dialog open={newTaskOpen} onOpenChange={setNewTaskOpen}>
        <DialogContent>
          ...
          <div className="py-4 space-y-4">
            <Input autoFocus value={taskTitle} onChange={e => setTaskTitle(e.target.value)} placeholder="What needs to be done?" />
          </div>
          <DialogFooter>
