...
export function ChallengesPage({ embedded = false }: { embedded?: boolean }) {
  ...
  const [activeTab, setActiveTab] = useState<"question" | "hints" | "solution" | "code">(
    "question",
  );
  ...
  const runUserCode = () => {
    ...
    setTimeout(() => {
      setCodeOutput({
        status: "success",
        message:
          "✓ All test cases passed! Complexity: Optimal O(N). Points earned: +" +
    ...
    <div className="max-w-6xl mx-auto px-4 py-8 space-y-6">
      ...
      {dailyChallenge && (
        <div className="rounded-2xl border border-emerald-500/30 bg-gradient-to-r from-emerald-500/10 via-card to-teal-500/10 p-6 shadow-md relative overflow-hidden">
          ...
          <p className="text-xs text-muted-foreground line-clamp-2 mb-4 leading-relaxed max-w-2xl">
          ...
          <Button
            size="sm"
            onClick={() => openChallengeModal(dailyChallenge)}
            className="rounded-xl font-bold cursor-pointer shadow-md"
          >
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground font-medium flex items-center gap-2 mt-0.5">
                      ...
                      <span className="text-amber-500 font-bold">
                        {selectedChallenge.points} pts reward
                      </span>
                    </p>
                  </div>
                </div>
                ...
                {activeTab === "question" && (
                  <div className="space-y-4 text-sm leading-relaxed">
                    <div className="bg-muted/20 border border-border/60 p-4 rounded-2xl">
                      <h4 className="font-bold text-foreground mb-1 text-xs uppercase tracking-wider">
                        Problem Statement
                      </h4>
                    ...
                    {selectedChallenge.examples?.length > 0 && (
                      <div className="space-y-3">
                        ...
                        {selectedChallenge.examples.map((ex: any, idx: number) => (
                          <div
                            key={idx}
                            className="bg-muted/40 border border-border/70 p-3.5 rounded-xl space-y-1 text-xs font-mono"
                          >
                            <p className="text-muted-foreground">
                              <strong className="text-foreground font-sans">Input:</strong>{" "}
                              {ex.input}
                            </p>
                            <p className="text-emerald-500 font-bold">
                              <strong className="text-foreground font-sans">Output:</strong>{" "}
                              {ex.output}
                            </p>
                            {ex.explanation && (
                    ...
                    <div className="space-y-3">
                      {(
                        ...
                        .map((hint: string, idx: number) => (
                          <div
                            key={idx}
                            className="bg-amber-500/10 border border-amber-500/30 p-4 rounded-2xl text-xs sm:text-sm font-medium text-foreground"
                          >
                  ...
                  <div className="space-y-4">
                    <div className="bg-emerald-500/10 border border-emerald-500/30 p-4 rounded-2xl text-xs sm:text-sm">
                      <h4 className="font-bold text-emerald-600 dark:text-emerald-400 text-sm mb-1">
                        Reference Solution & Complexity
                      </h4>
                      <p className="text-foreground/90 font-medium mb-3">
                        {selectedChallenge.solution?.explanation ||
                          "This is the optimal reference solution."}
                      </p>
                      <div className="flex gap-4 font-bold text-xs">
                        <span className="bg-card border border-border px-3 py-1 rounded-lg">
                          Time: {selectedChallenge.solution?.time_complexity || "O(N)"}
                        </span>
                        <span className="bg-card border border-border px-3 py-1 rounded-lg">
                          Space: {selectedChallenge.solution?.space_complexity || "O(1)"}
                        </span>
                      </div>
                    </div>
                    ...
                    <div className="bg-slate-950 text-slate-100 p-4 rounded-2xl border border-slate-800 font-mono text-xs overflow-x-auto leading-relaxed">
                      <pre>
                        {selectedChallenge.solution?.code || selectedChallenge.initial_code}
                      </pre>
                    </div>
                  </div>
                )}
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <Link
                        ...
                        search={{ challenge: selectedChallenge.slug } as any}
                      >
                        <Button
                          variant="outline"
                          size="sm"
