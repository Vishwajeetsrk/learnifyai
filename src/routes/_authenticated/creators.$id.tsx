...
import {
  Bell, BellOff, GraduationCap, Loader2, ArrowLeft, Users, Heart, Eye, BookOpen, TrendingUp,
  ExternalLink, Globe, Verified, BarChart3,
} from "lucide-react";
import {
...
function CreatorProfile() {
  ...
  const profileQuery = useQuery({
    ...
    queryFn: async () => {
      const [{ data: profile }, { data: courses, error: cErr }, { count: subs }] = await Promise.all([
        supabase.from("profiles").select("id, full_name, avatar_url, banner_url, bio, social_links").eq("id", id).maybeSingle(),
        supabase.from("courses").select("id, slug, title, description, cover_url, category, level, price_inr, duration_minutes").eq("created_by", id).eq("published", true).order("created_at", { ascending: false }),
        supabase.from("creator_subscriptions").select("*", { count: "exact", head: true }).eq("creator_id", id),
      ]);
      if (cErr) throw cErr;
  ...
  const subQuery = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase.from("creator_subscriptions").select("id").eq("subscriber_id", user!.id).eq("creator_id", id).maybeSingle();
      return !!data;
    },
  });
  ...
  const toggleSub = async () => {
    ...
    if (subQuery.data) {
      ...
      const { error } = await supabase.from("creator_subscriptions").insert({ subscriber_id: user.id, creator_id: id });
  ...
  return (
    <AppShell>
      <div className="px-4 sm:px-6 lg:px-10 py-6 max-w-6xl">
        <Link to="/courses" className="inline-flex items-center gap-1.5 text-xs text-muted-foreground hover:text-foreground mb-4 group">
          <ArrowLeft className="h-3.5 w-3.5 transition-transform group-hover:-translate-x-0.5" /> Back to courses
        </Link>
        ...
        <div className="rounded-3xl border bg-card overflow-hidden shadow-lg relative">
            ...
            {p?.profile?.banner_url ? (
              ...
              <div className="absolute inset-0">
                ...
                <div className="flex items-center justify-center h-full">
                  <span className="text-white/30 font-display font-medium tracking-widest text-lg uppercase">Creator</span>
                </div>
              </div>
            )}
          </div>
          ...
          <div className="px-6 pb-6 sm:px-8 sm:pb-8 flex flex-col sm:flex-row items-start gap-5 relative">
            <Avatar className="h-28 w-28 sm:h-32 sm:w-32 border-4 border-card rounded-full -mt-14 sm:-mt-16 shadow-xl bg-card shrink-0 ring-2 ring-primary/20">
              ...
              <AvatarFallback className="text-3xl sm:text-4xl font-display font-bold bg-gradient-to-br from-primary to-primary/60 text-white">{initials}</AvatarFallback>
            </Avatar>
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div className="min-w-0">
                  <div className="flex items-center gap-2.5">
                    <h1 className="font-display text-2xl sm:text-3xl font-bold tracking-tight">{name}</h1>
                    {isSelf && <Verified className="h-5 w-5 text-primary shrink-0" />}
                  </div>
                  <div className="flex flex-wrap items-center gap-3 mt-1.5 text-sm text-muted-foreground">
                    <span className="flex items-center gap-1.5">
                      <Users className="h-4 w-4 text-primary" />
                      <span className="font-medium text-foreground">{p?.subscribers ?? 0}</span> subscribers
                    </span>
                    <span className="text-muted-foreground/40">|</span>
                    <span><span className="font-medium text-foreground">{p?.courses.length ?? 0}</span> courses</span>
                  </div>
                </div>

                {!isSelf && (
                  <Button onClick={toggleSub} variant={subscribed ? "outline" : "default"} size="lg" className="rounded-full shadow-sm gap-2 w-full sm:w-auto">
                    {subscribed ? <><BellOff className="h-4 w-4" /> Subscribed</> : <><Bell className="h-4 w-4" /> Subscribe</>}
                  </Button>
                )}
              </div>

              {p?.profile?.bio && (
                <p className="mt-3 text-sm text-muted-foreground leading-relaxed max-w-3xl">{p.profile.bio}</p>
              )}
              ...
              {p?.profile?.social_links && typeof p.profile.social_links === "object" && (
                <div className="flex flex-wrap gap-2 mt-4">
                  {Object.entries(p.profile.social_links as Record<string, string>).map(([platform, url]) => {
                    if (!url) return null;
                    return (
                      <a key={platform} href={url.startsWith("http") ? url : `https://${url}`} target="_blank" rel="noreferrer"
        ...
        <div className="mt-12 mb-6 flex items-center gap-3">
          ...
          <Badge variant="secondary" className="text-xs">{p?.courses.length ?? 0}</Badge>
        </div>
            ...
            {p.courses.map((c, i) => (
              <Link key={c.id} to="/courses/$slug" params={{ slug: c.slug }}
                ...
                <div className="aspect-video bg-gradient-to-br from-muted to-muted/50 overflow-hidden relative">
                  {c.cover_url ? (
                    <img src={c.cover_url} alt="" className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" loading="lazy" />
                  ) : (
                  ...
                  <div className="absolute top-2 right-2">
                    <Badge variant="secondary" className="text-[10px] backdrop-blur-sm bg-background/80">{c.level}</Badge>
                  </div>
                </div>
  ...
  return (
    <section className="mt-12">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        <StatCard icon={Users} label="Subscribers" value={d.subscribers} trend="+" color="from-violet-500 to-purple-600" />
        <StatCard icon={Eye} label="Views" value={d.totalViews} trend="+" color="from-blue-500 to-cyan-500" />
        <StatCard icon={Heart} label="Likes" value={d.totalLikes} trend="+" color="from-rose-500 to-pink-500" />
        <StatCard icon={BookOpen} label="Enrollments" value={d.totalEnrollments} trend="+" color="from-emerald-500 to-teal-500" />
      </div>
      ...
      {d.courses.length > 0 && (
        <div className="rounded-2xl border bg-card shadow-sm overflow-hidden">
          ...
          <div className="p-4">
            <ResponsiveContainer width="100%" height={Math.max(240, d.courses.length * 48)}>
              <BarChart data={chartData} layout="vertical" margin={{ left: 8, right: 16, top: 8, bottom: 8 }}>
                ...
                <YAxis dataKey="name" type="category" width={130} stroke="hsl(var(--muted-foreground))" fontSize={11} tickLine={false} />
                <Tooltip contentStyle={{ background: "hsl(var(--card))", border: "1px solid hsl(var(--border))", borderRadius: 12, fontSize: 12, boxShadow: "0 4px 24px rgba(0,0,0,0.08)" }} />
          ...
          <div className="border-t">
            <div className="px-5 py-3 bg-muted/20 text-xs font-semibold uppercase tracking-wider text-muted-foreground">Course breakdown</div>
                  ...
                  {d.courses.map((c) => (
                    <tr key={c.id} className="border-b last:border-0 hover:bg-accent/30 transition-colors">
                      <td className="px-4 py-3">
                        <Link to="/courses/$slug" params={{ slug: c.slug }} className="hover:text-primary font-medium flex items-center gap-1.5">
                          {c.title} <ExternalLink className="h-3 w-3 text-muted-foreground opacity-0 group-hover:opacity-100" />
                        </Link>
                      </td>
    ...
    <div className="rounded-2xl border bg-card p-5 shadow-sm hover:shadow-md transition-all duration-200 hover:-translate-y-0.5 relative overflow-hidden group">
      ...
      <div className="relative">
        ...
        <div className="mt-1.5 flex items-baseline gap-1">
          ...
          {trend && value > 0 && <span className="text-xs text-emerald-500 font-medium">{trend}</span>}
        </div>
      </div>
    </div>
  );
