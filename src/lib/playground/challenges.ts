...
export const DEFAULT_CHALLENGES = [
  {
    ...
    solution: {
      ...
      explanation: "Iterate through the array once. For each number, calculate `diff = target - nums[i]`. Check if `diff` is in our Hash Map. If it is, return `[map.get(diff), i]`. Otherwise, store `nums[i]` with index `i` in the map.",
  ...
  },
  {
    ...
    solution: {
      ...
      explanation: "Normalize the string by stripping non-alphanumeric characters with regex and converting to lower case. Then check equality against its reversed string.",
  ...
  },
  {
    ...
    solution: {
      ...
      explanation: "Iterate through the list. For each node, store the next node temporarily, point current node's next to previous node, then move `prev` and `curr` forward.",
  ...
  },
  {
    ...
    solution: {
      ...
      explanation: "Maintain a sliding window `[left...right]`. Update `left` bound whenever duplicate character is observed to ensure all characters in window are unique.",
  ...
  },
  {
    ...
    solution: {
      ...
      explanation: "Traverse tree using queue BFS. For each level, dequeue `levelSize` nodes, push their values to `currentLevel`, and enqueue children.",
  ...
  },
  {
    ...
    solution: {
      ...
      explanation: "Merge lists pairwise using divide and conquer. In each pass, the number of lists is halved, giving logarithmic recursion depth.",
  ...
  },
  {
    ...
    examples: [
      { input: "nums = [1, 2, 3, 4, 5, 6]", output: "[4, 16, 36]" },
    ],
    hints: [
    ...
    solution: {
      ...
      explanation: "Single-line Python list comprehension that filters even numbers with `if x % 2 == 0` and squares them with `x**2`.",
  ...
  },
  {
    ...
    solution: {
      ...
      explanation: "Decorator wraps function call, uses `time.perf_counter()` for high resolution timing, prints execution time, and returns function result.",
  ...
  },
  {
    ...
    examples: [
      { input: "Employee table with salaries: [100, 200, 300]", output: "200" },
    ],
    hints: [
    ...
    solution: {
      ...
      explanation: "Subquery finds overall maximum salary. Outer query finds maximum salary strictly less than the subquery result.",
  ...
  },
  {
    ...
    examples: [
      { input: "Logins table with timestamps", output: "month | active_users" },
    ],
    hints: [
    ...
    solution: {
      code: "SELECT \n  DATE_TRUNC('month', login_time) AS month,\n  COUNT(DISTINCT user_id) as active_users\nFROM user_logins\nGROUP BY 1\nORDER BY month DESC;",
      explanation: "Group login events by month truncation and aggregate count of distinct user IDs to compute MAU stats.",
  ...
  },
  {
    ...
    examples: [
      { input: "Long URL: https://learnifyai.in/courses/advanced-system-design", output: "Short URL: https://lnfy.in/7xK9a" },
    ],
    hints: [
    ...
    solution: {
      ...
      explanation: "High throughput architecture using Base62 encoding, Redis caching, and PostgreSQL auto-increment ID generation with partition sharding.",
  ...
  },
  {
    ...
    examples: [
      { input: "Limit = 5 requests / 60 seconds per user IP", output: "6th request rejected with 429 Too Many Requests" },
    ],
    hints: [
    ...
    solution: {
      ...
      explanation: "Sliding Window Counter algorithm filters request timestamps dynamically within rolling window duration, preventing burst traffic bypass.",
