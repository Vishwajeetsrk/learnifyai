...
export const getCertificateStats = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    ...
    const verificationLogs = logs.filter(
      (l) => l.action === "verify" || l.action === "verified" || l.action === "scanned",
    );
    ...
    const { data: allCertsDates } = await supabaseAdmin.from("certificates").select("issued_at");
    ...
    const downloadedCount = logs.filter(
      (l) => l.action === "downloaded" || l.action === "saved",
    ).length;
...
export const getCertSettings = createServerFn({ method: "GET" })
  ...
  .handler(async () => {
    ...
    const settings: Record<string, string> = {
      ...
      cert_email_from: "support.learnifyai@gmail.com",
      cert_email_reply_to: "support.learnifyai@gmail.com",
