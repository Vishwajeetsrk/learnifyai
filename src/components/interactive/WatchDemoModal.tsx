...
import {
  X, Sparkles, Brain, DollarSign, BookOpen, GraduationCap,
...
const TOUR_STEPS = [
  {
    ...
    icon: Brain,
    features: [
      { num: "01", title: "AI Tutor & Smart Notes", desc: "24/7 AI tutor that explains complex concepts, generates instant quizzes, and creates structured study notes.", icon: Sparkles, color: "text-indigo-600", bg: "bg-indigo-500/10" },
      { num: "02", title: "Career Studio 5-in-1", desc: "Resume Builder, ATS Checker, Mock Interview Coach, Skill Gap Analyzer, and Portfolio Builder all integrated.", icon: Rocket, color: "text-purple-600", bg: "bg-purple-500/10" },
      { num: "03", title: "Verified QR Certificates", desc: "Earn tamper-proof certificates with QR code verification and 1-click LinkedIn Profile integration.", icon: GraduationCap, color: "text-emerald-600", bg: "bg-emerald-500/10" },
    ],
  },
  {
    id: "usage",
  ...
  },
  {
    ...
    icon: DollarSign,
    earnCards: [
      { title: "Course Creators", subtitle: "80/20 Revenue Split", desc: "Publish paid courses or cohort programs. Keep 80% of all sales with automated Cashfree payouts directly to your bank account.", items: ["Free or Paid Creator Plans", "Automated GST Invoicing & Tax handling"], color: "emerald", icon: DollarSign },
      { title: "1-on-1 Coaches & Mentors", subtitle: "Set Your Own Rates", desc: "Set your own hourly session rates for resume reviews, mock interviews, and career guidance. Instant wallet top-ups and payouts.", items: ["Free & Paid Coach Tiers", "Verified Mentor Badge & Student Ratings"], color: "purple", icon: GraduationCap },
    ],
  },
];
...
export function WatchDemoModal({ open, onOpenChange }: WatchDemoModalProps) {
  ...
  const goNext = () => { if (activeStep < TOUR_STEPS.length - 1) { setDirection(1); setActiveStep((p) => p + 1); } };
  const goPrev = () => { if (activeStep > 0) { setDirection(-1); setActiveStep((p) => p - 1); } };
        ...
        >
          ...
          <div className="bg-gradient-to-r from-indigo-900 via-purple-900 to-slate-900 p-3 sm:p-4 md:p-5 text-white relative shrink-0">
            ...
            <div className="relative flex items-start justify-between gap-3">
              ...
              <DialogPrimitive.Close className="p-1.5 rounded-lg bg-white/10 hover:bg-white/20 transition-colors shrink-0" aria-label="Close">
            ...
            <div className="p-3 sm:p-4 md:p-5 pb-6">
                    ...
                    {current.id === "overview" && (
                      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                        {current.features!.map((f, i) => (
                          <motion.div key={f.num} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.1 }}
                            className="p-4 rounded-xl border border-border bg-muted/30 hover:shadow-md transition-all group cursor-default"
                          >
                            <div className={`w-9 h-9 rounded-lg ${f.bg} ${f.color} flex items-center justify-center mb-2.5 font-bold text-sm`}>{f.num}</div>
                            <h4 className="font-semibold text-sm mb-1 group-hover:text-primary transition-colors">{f.title}</h4>
                    ...
                    {current.id === "usage" && (
                      <div className="space-y-2.5">
                        {current.steps!.map((s, i) => (
                          <motion.div key={s.step} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.08 }}
                            className="p-3 rounded-xl border bg-card flex items-center gap-3 hover:border-primary/30 transition-all"
                          >
                            <span className="text-[10px] font-bold text-primary bg-primary/10 px-2.5 py-1 rounded-md shrink-0">{s.step}</span>
                    ...
                    {current.id === "earnings" && (
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        {current.earnCards!.map((card, i) => (
                          <motion.div key={card.title} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.12 }}
                          ...
                          >
                            <div className={`flex items-center gap-2 ${card.color === "emerald" ? "text-emerald-600" : "text-purple-600"} font-bold text-xs mb-2`}>
                              <card.icon className="w-4 h-4" /> {card.title} ({card.subtitle})
                            </div>
                            <p className="text-xs text-muted-foreground leading-relaxed mb-2.5">{card.desc}</p>
                            <ul className="text-xs space-y-1.5">
                              {card.items.map((item) => (
                                <li key={item} className="flex items-center gap-2">
                                  <CheckCircle2 className={`w-3.5 h-3.5 shrink-0 ${card.color === "emerald" ? "text-emerald-500" : "text-purple-500"}`} />
                                  <span>{item}</span>
                                </li>
                              ))}
                            </ul>
              ...
              <div className="mt-3 pt-3 border-t flex flex-col sm:flex-row gap-2 items-stretch sm:items-center justify-between">
                <div className="flex items-center gap-1.5 justify-center sm:justify-start">
                  {TOUR_STEPS.map((_, i) => (
                    <div key={i} className={`h-1.5 rounded-full transition-all duration-300 ${i === activeStep ? "w-5 bg-primary" : i < activeStep ? "w-1.5 bg-primary/40" : "w-1.5 bg-muted"}`} />
                  ))}
                </div>
                <div className="flex items-center gap-2">
                  <span className="text-[10px] sm:text-xs text-muted-foreground flex-1 text-center sm:text-left">Ready to elevate your tech & career journey?</span>
                  <div className="flex items-center gap-1.5">
                    <Button variant="outline" size="sm" onClick={goPrev} disabled={activeStep === 0} className="h-7 w-7 p-0" aria-label="Previous">
                      <ChevronLeft className="h-4 w-4" />
                    </Button>
                    {activeStep < TOUR_STEPS.length - 1 ? (
                      <Button size="sm" onClick={goNext} className="h-7 text-xs">Next <ChevronRight className="h-3.5 w-3.5 ml-1" /></Button>
                    ) : (
                      <Button size="sm" onClick={() => onOpenChange(false)} className="h-7 text-xs">Close Tour <ArrowRight className="h-3.5 w-3.5 ml-1" /></Button>
                    )}
                  </div>
                </div>
              </div>
