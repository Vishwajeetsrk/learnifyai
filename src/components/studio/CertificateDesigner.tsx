...
type DecorStyle =
  "teal-sweep" | "blue-angular" | "purple-geometric" | "green-ribbons" | "crimson-ribbons";
...
const templates: Record<string, TemplateConfig> = {
  ...
  blue: {
    id: "blue",
    label: "Navy Blue",
    img: "/mockup/images/cert-28.png",
    primary: "#1E3A8A",
    accent: "#D4A843",
    certBg: "#FFFFFF",
    badgeBg: "#1E3A8A",
    badgeText: ["CERTIFICATE", "OF ACHIEVEMENT"],
    badgeIcon: "crown",
    decorStyle: "blue-angular",
    decorColor: "#1E3A8A",
    decorColor2: "#172554",
    verifyBg: "#1E3A8A",
    certIdColor: "#D4A843",
      ...
      {isSelected && (
        <div className="absolute -inset-2 border-[1.5px] border-blue-500 pointer-events-none z-50">
          {[
            ...
            <div
              key={i}
              className={`absolute ${pos} w-2.5 h-2.5 bg-white border border-blue-500 shadow-sm`}
            />
          ))}
...
const LaurelLeft = ({ color, size = 40 }: { color: string; size?: number }) => (
  <svg width={size} height={size * 1.6} viewBox="0 0 40 65" fill="none">
    ...
    {[
      ...
      <path
        key={i}
        d={`M${sx} ${sy} Q${(sx + ex) / 2 - 4} ${(sy + ey) / 2 - 2} ${ex} ${ey}`}
        stroke={color}
        strokeWidth="1.2"
        fill="none"
      />
    ...
    {[
      ...
      <ellipse
        ...
        fill={color}
        opacity="0.85"
        transform={`rotate(-30 ${(sx + ex) / 2 - 3} ${(sy + ey) / 2})`}
      />
    ))}
  </svg>
);
...
const BadgeIcon = ({
  ...
  if (type === "code")
    return (
      <span style={{ color, fontFamily: "monospace", fontWeight: 900, fontSize: size * 0.95 }}>
        &lt;/&gt;
      </span>
    );
...
const CertBgDecor = ({ t }: { t: TemplateConfig }) => {
  ...
  if (s === "teal-sweep")
    return (
      <>
        ...
        {/* Gold inset border */}
        <div
          className="absolute inset-[14px] border-[3.5px] pointer-events-none z-10"
  ...
  if (s === "blue-angular")
    return (
      <>
        ...
        {/* Gold border */}
        <div
          className="absolute inset-[14px] border-[2.5px] pointer-events-none z-10"
          style={{ borderColor: accent }}
        />
      </>
    );

  if (s === "purple-geometric")
    return (
      <>
        ...
        >
          ...
          <polygon
            points="280,290 100,290 200,40 380,40 380,110 300,110"
            fill={c2}
            opacity="0.75"
          />
        ...
        <div
          className="absolute top-5 right-5 w-2 h-2 rotate-45 pointer-events-none"
  ...
  if (s === "green-ribbons")
    return (
      <>
        ...
        {[
          ["top-0 left-0", "M4 30 L4 4 L30 4"],
          ["top-0 right-0", "M26 4 L50 4 L50 30"],
          ["bottom-0 left-0", "M4 20 L4 46 L30 46"],
          ["bottom-0 right-0", "M26 46 L50 46 L50 20"],
        ].map(([pos, d], i) => (
          <svg
            key={i}
        ...
        <div
          className="absolute inset-[14px] border-[2px] pointer-events-none z-10"
          style={{ borderColor: accent }}
        />
      </>
    );
  ...
  return (
    <>
      ...
      {/* L-bracket corners */}
      {[
        ["top-0 left-0", "M4 28 L4 4 L28 4"],
        ["top-0 right-0", "M24 4 L48 4 L48 28"],
        ["bottom-0 left-0", "M4 18 L4 44 L28 44"],
        ["bottom-0 right-0", "M24 44 L48 44 L48 18"],
      ].map(([pos, d], i) => (
        <svg
          key={i}
          className={`absolute ${pos} pointer-events-none`}
...
const BadgeSeal = ({ t, className }: { t: TemplateConfig; className?: string }) => (
  <div className={`relative flex flex-col items-center ${className ?? ""}`}>
    ...
    <svg
      ...
      viewBox="0 0 64 190"
      fill="none"
      className="absolute top-0 z-10 drop-shadow-md"
    >
    ...
    <div className="absolute top-[120px] z-20 w-[100px] h-[100px] flex items-center justify-center">
      <svg width="100" height="100" viewBox="0 0 100 100" fill="none" className="absolute inset-0">
        ...
        {[-12, 0, 12].map((x) => (
          <polygon
            key={x}
            points={`${50 + x},72 ${52 + x},77 ${57 + x},77 ${53 + x},80 ${55 + x},85 ${50 + x},82 ${45 + x},85 ${47 + x},80 ${43 + x},77 ${48 + x},77`}
            fill={t.accent}
            transform={`scale(0.55) translate(${(50 + x) * 0.82},${72 * 0.82})`}
          />
        ))}
      </svg>
      {/* Icon + text in center */}
      <div className="relative z-10 flex flex-col items-center">
        ...
        {t.badgeText.map((line, i) => (
          <span
            key={i}
            className="text-[7px] font-black tracking-[0.12em] uppercase leading-tight text-center"
        ...
        <div className="flex gap-1 mt-1">
          {[0, 1, 2].map((i) => (
            <Star key={i} size={6} fill={t.accent} style={{ color: t.accent }} />
          ))}
        </div>
      </div>
    </div>
...
const CenterSeal = ({ t }: { t: TemplateConfig }) => (
  <div className="w-[110px] h-[110px] relative flex items-center justify-center">
    <svg width="110" height="110" viewBox="0 0 110 110" fill="none" className="absolute inset-0">
      ...
      {[
        ...
        <ellipse
          ...
          fill={t.accent}
          opacity="0.85"
          transform={`rotate(-40 ${(sx + ex) / 2 - 4} ${(sy + ey) / 2})`}
        />
      ))}
      {/* Right side (mirrored) */}
      {[
      ...
      {[43, 55, 67].map((x) => (
        <polygon
          key={x}
          points={`${x},88 ${x + 1.5},91.5 ${x + 5},91.5 ${x + 2},93.5 ${x + 3.5},97 ${x},95 ${x - 3.5},97 ${x - 2},93.5 ${x - 5},91.5 ${x - 1.5},91.5`}
          fill={t.accent}
        />
      ))}
    </svg>
    <img
      src="/images/logo.png"
      alt="Logo"
      className="h-8 brightness-0 invert opacity-90 relative z-10"
    />
  </div>
);
...
const QRCodeDisplay = ({ url, size = 90 }: { url: string; size?: number }) => {
  ...
  return (
    <div
      className="bg-white p-1 border border-gray-200 rounded shadow-sm flex items-center justify-center"
...
const elementMeta: Record<
  string,
  { label: string; font?: string; size?: number; variable?: string }
> = {
...
export function CertificateDesigner() {
  ...
  const updateField = (key: string, val: string) => setContentData((d) => ({ ...d, [key]: val }));
  ...
  const handleDownloadPDF = useCallback(async () => {
    ...
    if (canvas) {
      ...
      pdf.save(
        `certificate-${contentData.student_name.replace(/\s+/g, "_")}-${contentData.cert_id}.pdf`,
      );
    ...
    <div className="flex flex-col h-screen w-full bg-[#0B1929] text-slate-200 overflow-hidden font-sans">
      ...
      <div className="h-12 bg-[#0B1929] border-b border-slate-800 flex items-center justify-between px-4 shrink-0 z-20">
        <div className="flex items-center gap-2 sm:gap-3">
          <div className="w-6 h-6 rounded bg-gradient-to-br from-teal-400 to-blue-600 flex items-center justify-center font-black text-white text-xs">
            L
          </div>
        ...
        <div className="flex items-center gap-0.5 hidden md:flex">
          {["File", "Edit", "View", "Insert", "Templates", "Brand Kit", "AI Tools", "Resize"].map(
            (m) => (
              <button
                key={m}
                className="px-2.5 py-1.5 text-[11px] font-medium text-slate-300 hover:text-white hover:bg-slate-800 rounded transition-colors"
              >
                {m}
              </button>
            ),
        ...
        <div className="flex items-center gap-2 sm:gap-3">
          ...
          <div className="flex items-center gap-1">
            ...
            <button
              ...
              className="px-2.5 py-1.5 text-xs font-bold text-[#0B1929] bg-[#D4A843] hover:bg-[#e8c06a] rounded-r border-l border-[#b88e35] transition-colors"
            >
              PDF
            </button>
          </div>
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        {/* ── LEFT CONTENT PANEL ── */}
        <div
          className={`w-[276px] bg-slate-900 border-r border-slate-800 flex-col shrink-0 z-20 transition-all duration-200 ${
            mobileLeftOpen
          ...
          {mobileLeftOpen && (
            <div className="p-2 border-b border-slate-800 flex justify-end lg:hidden shrink-0">
              <button
                ...
                className="text-slate-400 hover:text-white text-xs font-semibold px-2 py-1 border border-slate-800 rounded"
              >
                Close ×
              </button>
            </div>
              ...
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                <div className="flex justify-between items-end mb-3">
                  <div>
                    ...
                    <p className="text-[10px] text-slate-500 mt-0.5">
                      5 active &middot; 5 coming soon
                    </p>
                  </div>
                  <button className="text-[10px] text-yellow-400 hover:text-yellow-300 font-semibold">
                    Browse All
                  </button>
                </div>
                <div className="grid grid-cols-2 gap-2.5 mb-4">
                  {Object.values(templates).map((tmpl) => (
                    ...
                    >
                      <div
                        className={`aspect-[1.41] rounded overflow-hidden border-2 transition-all ${selectedTemplate === tmpl.id ? "border-yellow-400 shadow-[0_0_12px_rgba(250,204,21,0.25)]" : "border-slate-700 hover:border-slate-500"} relative`}
                      >
                        <img
                          src={tmpl.img}
                          alt={tmpl.label}
                      ...
                      <p
                        className={`text-[10px] font-semibold text-center mt-1.5 ${selectedTemplate === tmpl.id ? "text-yellow-400" : "text-slate-400 group-hover:text-slate-200"}`}
                      >
                        {tmpl.label}
                      </p>
                    </div>
                ...
                {/* Coming soon slots */}
                <h3 className="text-[9px] font-bold text-slate-600 uppercase tracking-wider mb-2">
                  Coming Soon
                </h3>
                <div className="grid grid-cols-2 gap-2">
                  {comingSoon.map((cs) => (
                    <div
                      key={cs.id}
                      className="aspect-[1.41] rounded border border-dashed border-slate-700 flex flex-col items-center justify-center gap-1 cursor-pointer hover:border-slate-500 transition-colors"
                    >
                ...
                <div>
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    {[Award, Star, Shield].map((Icon, i) => (
                      <div
                        key={i}
                        className="aspect-square bg-slate-800 rounded border border-slate-700 flex items-center justify-center hover:border-yellow-400 cursor-pointer group transition-colors"
                      >
                ...
                <div>
                  ...
                  <div className="space-y-2">
                    {["M0 8 H15 L20 2 L25 8 H40", "M0 8 Q20 2 40 8", "M0 8 H40"].map((d, i) => (
                      <div
                        key={i}
                        className="h-10 bg-slate-800 rounded border border-slate-700 flex items-center justify-center hover:border-yellow-400 cursor-pointer group transition-colors"
                      >
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {["Bracket", "Ornamental", "Curved", "Minimal"].map((name) => (
                      <div
                        key={name}
                        className="h-14 bg-slate-800 rounded border border-slate-700 flex items-center justify-center hover:border-yellow-400 cursor-pointer group transition-colors text-[9px] text-slate-500 group-hover:text-yellow-400"
                      >
                        {name}
                      </div>
                    ))}
            ...
            {activeTab === "text" && (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-2.5">
                {[
                  {
                    preview: "CERTIFICATE",
                    cls: "font-playfair text-[18px] tracking-[0.15em] font-semibold",
                    label: "Title – Playfair Display",
                  },
                  {
                    preview: "Student Name",
                    cls: "font-great-vibes text-[26px] text-yellow-400",
                    label: "Name – Great Vibes",
                  },
                  {
                    preview: "Course Title Bold",
                    cls: "font-sans text-[14px] font-bold text-slate-200",
                    label: "Course – Sans Bold",
                  },
                  {
                    preview: "Body / certify text",
                    cls: "font-playfair italic text-[13px] text-slate-300",
                    label: "Italic Body",
                  },
                  {
                    preview: "LABEL TEXT",
                    cls: "font-sans text-[9px] tracking-[0.2em] font-bold text-slate-400",
                    label: "Small Caps",
                  },
                ].map((s, i) => (
                  <button
                    key={i}
                    className="w-full py-3 px-3 bg-slate-800 border border-slate-700 rounded hover:border-slate-500 text-left transition-colors"
                  >
        ...
        <div className="flex-1 checkerboard-bg relative overflow-hidden flex flex-col">
          ...
          <div className="absolute top-3 left-1/2 -translate-x-1/2 bg-white rounded-lg shadow-md border border-gray-200 p-0.5 flex items-center gap-0.5 z-10">
            <div
              className="flex items-center gap-1 px-2 py-1 bg-gray-50 rounded border border-gray-200 cursor-pointer hover:bg-gray-100 transition-colors"
              onClick={() => setZoom((v) => Math.max(30, v - 10))}
            >
              <span className="text-[11px] font-semibold text-gray-700 w-10 text-center">
                {zoom}%
              </span>
          ...
          <div
            ref={containerRef}
            className="flex-1 overflow-auto flex items-center justify-center p-4 sm:p-8 md:p-16"
          ...
          >
            ...
            >
              ...
              <Selectable
                ...
                className="top-0 left-[68px] w-[110px] h-[240px]"
              >
                <BadgeSeal t={t} />
              </Selectable>
              ...
              <Selectable
                ...
                selectedId={selectedElement}
                onSelect={setSelectedElement}
                className="top-[28px] left-1/2 -translate-x-1/2 flex flex-col items-center gap-1"
              >
                <img src="/images/logo.png" alt="Learnify AI" className="h-12" />
                <div className="flex items-center gap-2">
                  <div className="h-px w-12" style={{ background: t.accent }} />
                  <span className="text-[10px] text-gray-500 tracking-[0.15em] font-semibold">
              ...
              <Selectable
                ...
                selectedId={selectedElement}
                onSelect={setSelectedElement}
                className="top-[32px] right-[46px] text-right"
              >
              ...
              <Selectable
                ...
                selectedId={selectedElement}
                onSelect={setSelectedElement}
                className="top-[112px] w-full flex flex-col items-center"
              >
                <h1
                  className="font-playfair tracking-[0.18em] font-semibold leading-none mb-2.5"
                  style={{ color: t.primary, fontSize: 62 }}
                >
                  CERTIFICATE
                </h1>
                <div className="flex items-center gap-3">
                  ...
                  <span
                    className="tracking-[0.3em] text-[11.5px] font-bold"
                    style={{ color: t.accent }}
                  >
                    OF COMPLETION
                  </span>
              ...
              <Selectable
                ...
                selectedId={selectedElement}
                onSelect={setSelectedElement}
                className="top-[240px] w-full text-center"
              >
                <p className="font-playfair italic text-gray-500" style={{ fontSize: 15 }}>
              ...
              <Selectable
                ...
                selectedId={selectedElement}
                onSelect={setSelectedElement}
                className="top-[262px] w-full flex justify-center items-center gap-2"
              >
                <LaurelLeft color={t.accent} size={34} />
                <div className="relative">
                  <h2
                    className="font-great-vibes leading-none whitespace-nowrap"
                    style={{ color: t.nameColor, fontSize: 74 }}
                  >
                    {contentData.student_name}
                  </h2>
                  <div
                    className="absolute bottom-[-4px] left-0 right-0 h-[1.5px]"
                    style={{
                      background: `linear-gradient(to right, transparent, ${t.accent}, transparent)`,
                    }}
                  />
                  <div
                    className="absolute bottom-[-6px] left-1/2 -translate-x-1/2 w-1.5 h-1.5 rotate-45"
              ...
              <Selectable
                ...
                selectedId={selectedElement}
                onSelect={setSelectedElement}
                className="top-[378px] w-full text-center"
              >
                <p className="text-gray-500 tracking-wide" style={{ fontSize: 13 }}>
              ...
              <Selectable
                ...
                selectedId={selectedElement}
                onSelect={setSelectedElement}
                className="top-[400px] w-full text-center"
              >
                <h3
                  className="font-bold tracking-[0.03em]"
                  style={{ color: t.primary, fontSize: 23 }}
                >
              ...
              <Selectable
                ...
                selectedId={selectedElement}
                onSelect={setSelectedElement}
                className="top-[436px] w-full text-center px-[140px]"
              >
              ...
              <Selectable
                ...
                selectedId={selectedElement}
                onSelect={setSelectedElement}
                className="bottom-[72px] left-[78px] w-[185px] text-center"
              >
                <div
                  className="font-great-vibes -rotate-2 mb-0.5"
                  style={{ color: t.nameColor, fontSize: 34 }}
                >
                  {contentData.instructor_name}
                </div>
                <div className="h-[1px] w-full bg-gray-300 mb-1.5" />
                <div className="font-bold text-gray-700 text-[11px] tracking-wide uppercase">
                  {contentData.instructor_name}
                </div>
                <div className="text-[9.5px] text-gray-500 tracking-wider mt-0.5">
                  {contentData.instructor_title}
                </div>
              </Selectable>
              ...
              <Selectable
                ...
                selectedId={selectedElement}
                onSelect={setSelectedElement}
                className="bottom-[46px] left-1/2 -translate-x-1/2"
              >
                <CenterSeal t={t} />
              </Selectable>
              ...
              <Selectable
                ...
                selectedId={selectedElement}
                onSelect={setSelectedElement}
                className="bottom-[72px] right-[240px] w-[145px] text-center"
              >
                <div
                  className="font-bold text-gray-800 mb-1.5 h-9 flex items-end justify-center tracking-wide"
                  style={{ fontSize: 14 }}
                >
                  {contentData.completion_date}
                </div>
                <div className="h-[1px] w-full bg-gray-300 mb-1.5" />
                <div className="text-[9.5px] text-gray-500 uppercase tracking-widest font-semibold">
                  Date of Completion
                </div>
              </Selectable>
              ...
              <Selectable
                ...
                selectedId={selectedElement}
                onSelect={setSelectedElement}
                className="bottom-[56px] right-[46px] flex flex-col items-center gap-1.5"
              >
                <QRCodeDisplay url={contentData.verification_link} size={86} />
                <button
                  className="text-white text-[10px] font-bold px-3 py-1 rounded w-full text-center"
                  style={{ background: t.verifyBg }}
                >
                  Verify Certificate
                </button>
              </Selectable>
              ...
              <Selectable
                ...
                selectedId={selectedElement}
                onSelect={setSelectedElement}
                className="bottom-0 left-0 right-0 h-[50px]"
              >
                  ...
                  {t.features.map((f, i) => (
                    <React.Fragment key={i}>
                      ...
                      <div className="flex-1 flex items-center gap-2 px-3">
                        <div style={{ color: t.primary }} className="shrink-0">
                          <FeatureIcon icon={f.icon} size={13} />
                        </div>
                        <div>
        ...
        {/* ── RIGHT PANEL ── */}
        <div
          className={`w-[290px] bg-slate-900 border-l border-slate-800 flex-col shrink-0 z-20 transition-all duration-200 ${
            mobileRightOpen
          ...
          <div className="flex-1 overflow-y-auto">
                ...
                <div className="bg-slate-800 rounded-lg p-3">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[12px] font-semibold text-white">
                      {selectedMeta?.label ?? "Nothing selected"}
                    </span>
                  </div>
                </div>
                ...
                {selectedMeta?.variable && (
                  <div>
                    ...
                    <div className="bg-slate-800 rounded-lg p-2.5 flex items-center justify-between">
                      ...
                      <span className="text-[11px] font-mono font-bold text-yellow-400 bg-yellow-400/10 px-2 py-0.5 rounded">
                        {selectedMeta.variable}
                      </span>
                    </div>
                ...
                {selectedMeta?.font && (
                  <div>
                    ...
                    <div className="bg-slate-800 border border-slate-700 rounded-lg px-3 py-2 flex items-center justify-between cursor-pointer hover:border-slate-500">
                      <span className="text-[12px] text-slate-200 font-playfair">
                        {selectedMeta.font}
                      </span>
                    ...
                    <div className="flex gap-2 mt-2">
                      ...
                      {["B", "I", "U"].map((b) => (
                        <button
                          key={b}
                          className={`w-8 h-8 rounded border border-slate-700 text-[11px] font-bold hover:border-slate-500 transition-colors ${b === "B" ? "bg-slate-700 text-white" : "text-slate-400"}`}
                        >
                          {b}
                        </button>
                      ))}
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {[
                      ...
                      <div
                        key={label}
                        className="bg-slate-800 border border-slate-700 rounded px-2.5 py-1.5 flex items-center gap-1.5"
                      >
                ...
                <div>
                  ...
                  <div className="flex gap-1">
                    {[AlignLeft, AlignCenter, AlignRight, AlignJustify].map((Icon, i) => (
                      <button
                        key={i}
                        className={`flex-1 py-1.5 rounded border transition-colors ${i === 1 ? "border-yellow-400/50 bg-yellow-400/10 text-yellow-400" : "border-slate-700 text-slate-500 hover:text-slate-300 hover:border-slate-500"}`}
                      >
                  ...
                  <div className="flex gap-2 flex-wrap">
                    {Object.values(templates).map((tmpl) => (
                      <button
                        ...
                        style={{
                          background: tmpl.primary,
                          borderColor: selectedTemplate === tmpl.id ? "#D4A843" : "#475569",
                          boxShadow:
                            selectedTemplate === tmpl.id
            ...
            {rightTab === "content" && (
              <div className="p-4 space-y-4">
                <div>
                  <h3 className="text-[9px] font-bold text-slate-400 uppercase tracking-wider mb-3">
                    Certificate Fields
                  </h3>
                    ...
                    {Object.values(templates).map((tmpl) => (
                      ...
                      >
                        <div
                          ...
                          style={{ background: tmpl.primary }}
                        />
                        <span className="text-[11px] font-medium text-slate-200">{tmpl.label}</span>
                        {selectedTemplate === tmpl.id && (
                          <CheckCircle2 size={13} className="text-yellow-400 ml-auto shrink-0" />
                        )}
                      </button>
                    ))}
                  </div>
