...
import { Download, Save, Undo, Redo, Sparkles, Type, Image as ImageIcon, QrCode, ShieldCheck, PenTool } from "lucide-react";
    ...
    <div className="h-14 border-b bg-card flex items-center justify-between px-4 shrink-0 shadow-sm z-10">
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="flex items-center gap-1">
          <Button variant="ghost" size="sm" className="h-8 px-2" onClick={() => onAddElement('text')}><Type className="h-4 w-4 mr-1.5"/> Text</Button>
          <Button variant="ghost" size="sm" className="h-8 px-2" onClick={() => onAddElement('image')}><ImageIcon className="h-4 w-4 mr-1.5"/> Image</Button>
          <Button variant="ghost" size="sm" className="h-8 px-2" onClick={() => onAddElement('signature')}><PenTool className="h-4 w-4 mr-1.5"/> Signature</Button>
          <Button variant="ghost" size="sm" className="h-8 px-2" onClick={() => onAddElement('qr')}><QrCode className="h-4 w-4 mr-1.5"/> QR</Button>
        </div>
      </div>

      <div className="flex items-center gap-2">
        <Button variant="secondary" size="sm" onClick={onAiOptimize} className="h-8 bg-indigo-50 text-indigo-600 hover:bg-indigo-100 border border-indigo-200">
