...
export function PropertiesPanel({
  ...
  onUpdateDesign, onUpdateBgImageUrl, onDeleteElement, onDuplicateElement,
  onAddElement, onAddShape, onAddSvg, onAddDivider, onUploadImage,
}: PropertiesPanelProps) {
  const selectedEl = elements.find((e) => e.id === selectedId);
  const [expandedSections, setExpandedSections] = useState<Record<string, boolean>>({
    text: true, position: true, style: true, colors: true, typography: true,
    layout: true, assets: true, addElements: true, svgPicker: false,
  });
    ...
    <div className="w-[380px] bg-white border-l border-slate-200 flex flex-col h-full overflow-hidden shrink-0">
      <ScrollArea className="flex-1">
        <div className="p-4 space-y-6">
          {/* ADD ELEMENTS */}
          <div>
            <button onClick={() => toggle("addElements")} className="flex items-center gap-2 text-xs font-bold text-slate-900 w-full mb-3">
              {expandedSections.addElements ? <ChevronDown className="h-3.5 w-3.5" /> : <ChevronRight className="h-3.5 w-3.5" />}
          ...
          {selectedEl && (
            <div className="border-t border-slate-100 pt-4 space-y-4">
              <div className="flex items-center justify-between">
                <span className="text-xs font-bold text-slate-900 uppercase">Element Properties</span>
                <div className="flex items-center gap-1">
                  <Button variant="ghost" size="icon" className="h-6 w-6" onClick={() => onDuplicateElement(selectedEl.id)} title="Duplicate">
                    <Copy className="h-3 w-3" />
                  </Button>
                  <Button variant="ghost" size="icon" className="h-6 w-6 text-red-500" onClick={() => onDeleteElement(selectedEl.id)} title="Delete">
                ...
                <div className="space-y-3">
                  <div>
                    ...
                    <Textarea value={selectedEl.content || ""}
                      ...
                      className="mt-1 text-xs min-h-[60px]" rows={3} />
                  </div>
                  <div>
                    <Label className="text-xs text-slate-500">Font Family</Label>
                    <Select value={selectedEl.fontFamily || design.font_family}
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      <Label className="text-xs text-slate-500">Size: {selectedEl.fontSize}px</Label>
                      <Slider value={[selectedEl.fontSize || 16]} min={8} max={200}
                        onValueChange={(v) => onUpdateElement(selectedEl.id, { fontSize: v[0] })} className="mt-1" />
                    </div>
                    <div>
                      <Label className="text-xs text-slate-500">Color</Label>
                      <div className="flex gap-2 items-center mt-1">
                        <input type="color" value={selectedEl.color || design.text_color}
                  ...
                  <div className="grid grid-cols-4 gap-1 bg-slate-100 p-1 rounded-lg">
                    <Button variant={selectedEl.align === "left" ? "secondary" : "ghost"} size="sm" className="h-7"
                      onClick={() => onUpdateElement(selectedEl.id, { align: "left" })}>
                      <AlignLeft className="h-3.5 w-3.5" />
                    </Button>
                    <Button variant={selectedEl.align === "center" ? "secondary" : "ghost"} size="sm" className="h-7"
                      onClick={() => onUpdateElement(selectedEl.id, { align: "center" })}>
                      <AlignCenter className="h-3.5 w-3.5" />
                    </Button>
                    <Button variant={selectedEl.align === "right" ? "secondary" : "ghost"} size="sm" className="h-7"
                      onClick={() => onUpdateElement(selectedEl.id, { align: "right" })}>
                      <AlignRight className="h-3.5 w-3.5" />
                    </Button>
                    <Button variant={selectedEl.fontWeight === "bold" ? "secondary" : "ghost"} size="sm" className="h-7"
              ...
              {/* IMAGE PROPERTIES */}
              {(selectedEl.type === "image" || selectedEl.type === "org_logo" || selectedEl.type === "signature" || selectedEl.type === "badge") && (
                <div className="space-y-3">
                  <Label className="text-xs text-slate-500">Image Source</Label>
                  <div className="flex gap-2">
                    <Input value={selectedEl.url || ""}
                    ...
                    <label className="cursor-pointer inline-flex items-center justify-center h-8 px-3 bg-slate-100 hover:bg-slate-200 rounded-lg border text-xs font-medium shrink-0">
                      ...
                      <input type="file" accept="image/*" className="hidden"
                        onChange={(e) => { const f = e.target.files?.[0]; if (f) onUpdateElement(selectedEl.id, { url: URL.createObjectURL(f) }); }} />
                    </label>
                  </div>
                </div>
              )}
              ...
              {selectedEl.type === "svg" && (
                <div className="space-y-3">
                  ...
                  <Textarea value={selectedEl.svgContent || ""}
                    onChange={(e) => onUpdateElement(selectedEl.id, { svgContent: e.target.value })}
                    className="text-xs font-mono min-h-[80px]" rows={4} />
                  <div>
                    <Label className="text-xs text-slate-500">SVG Color</Label>
                    <div className="flex gap-2 items-center mt-1">
                      <input type="color" value={selectedEl.svgColor || design.accent_color}
                        onChange={(e) => onUpdateElement(selectedEl.id, { svgColor: e.target.value })}
                        className="w-8 h-8 rounded cursor-pointer border border-slate-200" />
                    </div>
                  </div>
                </div>
              ...
              {selectedEl.type === "shape" && (
                <div className="space-y-3">
                  ...
                  <Select value={selectedEl.shapeType || "rect"}
                    onValueChange={(v) => onUpdateElement(selectedEl.id, { shapeType: v as ShapeType })}>
                    <SelectTrigger className="h-8 text-xs"><SelectValue /></SelectTrigger>
                    <SelectContent>
                      {SHAPE_OPTIONS.map((s) => <SelectItem key={s.type} value={s.type}>{s.icon} {s.label}</SelectItem>)}
                    </SelectContent>
                  </Select>
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      ...
                      <div className="flex gap-2 items-center mt-1">
                        <input type="color" value={selectedEl.fillColor || "ffffff"}
                          onChange={(e) => onUpdateElement(selectedEl.id, { fillColor: e.target.value })}
                          className="w-8 h-8 rounded cursor-pointer border border-slate-200" />
                      </div>
                    </div>
                    <div>
                      ...
                      <div className="flex gap-2 items-center mt-1">
                        <input type="color" value={selectedEl.strokeColor || design.accent_color}
                          onChange={(e) => onUpdateElement(selectedEl.id, { strokeColor: e.target.value })}
                          className="w-8 h-8 rounded cursor-pointer border border-slate-200" />
                      </div>
                    </div>
                  </div>
              ...
              {selectedEl.type === "date" && (
                <div className="space-y-3">
                  ...
                  <Select value={selectedEl.dateFormat || "MMMM D, YYYY"}
                    onValueChange={(v) => onUpdateElement(selectedEl.id, { dateFormat: v })}>
                    <SelectTrigger className="h-8 text-xs"><SelectValue /></SelectTrigger>
                    <SelectContent>
                      {["YYYY-MM-DD", "MM/DD/YYYY", "DD/MM/YYYY", "MMMM D, YYYY", "MMM D, YYYY", "D MMMM YYYY"].map(f => (
                  ...
                  <div>
                    ...
                    <Input value={selectedEl.label || ""}
                      ...
                      placeholder="Date" className="mt-1 text-xs" />
                  </div>
                </div>
              )}
                ...
                <div className="grid grid-cols-2 gap-2">
                  ...
                  <div>
                    ...
                    <Input type="number" value={Math.round(selectedEl.width || 0)}
                      onChange={(e) => onUpdateElement(selectedEl.id, { width: parseInt(e.target.value) || undefined })}
                      className="h-7 text-xs font-mono mt-1" />
                  </div>
                  <div>
                    <Label className="text-[10px] text-slate-400">Height</Label>
                    <Input type="number" value={Math.round(selectedEl.height || 0)}
                      onChange={(e) => onUpdateElement(selectedEl.id, { height: parseInt(e.target.value) || undefined })}
                      className="h-7 text-xs font-mono mt-1" />
                  </div>
                </div>
                <div>
                  <Label className="text-[10px] text-slate-400">Rotation: {Math.round(selectedEl.rotation || 0)}°</Label>
                  <Slider value={[selectedEl.rotation || 0]} min={-180} max={180}
                    onValueChange={(v) => onUpdateElement(selectedEl.id, { rotation: v[0] })} className="mt-1" />
                </div>
                <div>
                  <Label className="text-[10px] text-slate-400">Opacity: {Math.round((selectedEl.opacity ?? 1) * 100)}%</Label>
                  <Slider value={[(selectedEl.opacity ?? 1) * 100]} min={0} max={100}
                    onValueChange={(v) => onUpdateElement(selectedEl.id, { opacity: v[0] / 100 })} className="mt-1" />
                </div>
              </div>
            </div>
          )}
          ...
          <div className="border-t border-slate-100 pt-4">
            <button onClick={() => toggle("colors")} className="flex items-center gap-2 text-xs font-bold text-slate-900 w-full mb-3">
              {expandedSections.colors ? <ChevronDown className="h-3.5 w-3.5" /> : <ChevronRight className="h-3.5 w-3.5" />}
              ...
              <div className="space-y-3">
                <div className="grid grid-cols-3 gap-1.5">
                  {COLOR_PALETTES.map((p) => (
                    <button key={p.id} onClick={() => onUpdateDesign({
                      accent_color: p.accent, bg_color: p.bg, text_color: p.text,
                ...
                <div className="grid grid-cols-2 gap-3 pt-2">
                  <div>
                    ...
                    <div className="flex gap-2 items-center mt-1">
                      <input type="color" value={design.accent_color}
                        ...
                        className="w-8 h-8 rounded cursor-pointer border border-slate-200" />
                      <Input value={design.accent_color}
                        onChange={(e) => onUpdateDesign({ accent_color: e.target.value })}
                  ...
                  <div>
                    ...
                    <div className="flex gap-2 items-center mt-1">
                      <input type="color" value={design.accent_color_2 || "#8a6d2b"}
                        onChange={(e) => onUpdateDesign({ accent_color_2: e.target.value })}
                        className="w-8 h-8 rounded cursor-pointer border border-slate-200" />
                      <Input value={design.accent_color_2 || ""}
                        onChange={(e) => onUpdateDesign({ accent_color_2: e.target.value })}
                        className="h-7 text-xs font-mono" placeholder="#8a6d2b" />
                    </div>
                  </div>
                  <div>
                    <Label className="text-xs text-slate-500">Background</Label>
                    <div className="flex gap-2 items-center mt-1">
                      <input type="color" value={design.bg_color}
                        onChange={(e) => onUpdateDesign({ bg_color: e.target.value })}
                        className="w-8 h-8 rounded cursor-pointer border border-slate-200" />
                      <Input value={design.bg_color}
                        onChange={(e) => onUpdateDesign({ bg_color: e.target.value })}
                        className="h-7 text-xs font-mono" />
                    </div>
                  </div>
                  <div>
                    ...
                    <div className="flex gap-2 items-center mt-1">
                      <input type="color" value={design.text_color}
                        onChange={(e) => onUpdateDesign({ text_color: e.target.value })}
                        className="w-8 h-8 rounded cursor-pointer border border-slate-200" />
                      <Input value={design.text_color}
                        onChange={(e) => onUpdateDesign({ text_color: e.target.value })}
                        className="h-7 text-xs font-mono" />
                    </div>
                  </div>
          ...
          {/* TYPOGRAPHY */}
          <div className="border-t border-slate-100 pt-4">
            <button onClick={() => toggle("typography")} className="flex items-center gap-2 text-xs font-bold text-slate-900 w-full mb-3">
            ...
            {expandedSections.typography && (
              <div className="space-y-3">
                <div>
                  ...
                  <Select value={design.title_font || design.font_family}
                ...
                <div>
                  ...
                  <Select value={design.body_font || design.font_family}
          ...
          {/* LAYOUT & DECORATION */}
          <div className="border-t border-slate-100 pt-4">
            <button onClick={() => toggle("layout")} className="flex items-center gap-2 text-xs font-bold text-slate-900 w-full mb-3">
            ...
            {expandedSections.layout && (
              <div className="space-y-3">
                ...
                <div>
                  ...
                  <Select value={design.corner_style} onValueChange={(v) => onUpdateDesign({ corner_style: v })}>
                    <SelectTrigger className="mt-1 h-8 text-xs"><SelectValue /></SelectTrigger>
                    <SelectContent>
                      {["none", "diagonal", "ribbon"].map((c) => <SelectItem key={c} value={c}>{c}</SelectItem>)}
                    </SelectContent>
                  </Select>
                </div>
                {design.border_style !== "none" && (
                  <div>
                    ...
                    <Slider value={[design.border_width]} min={1} max={40}
                      onValueChange={(v) => onUpdateDesign({ border_width: v[0] })} className="mt-1" />
                  </div>
                )}
              </div>
            )}
          ...
          {/* ASSETS */}
          <div className="border-t border-slate-100 pt-4">
            <button onClick={() => toggle("assets")} className="flex items-center gap-2 text-xs font-bold text-slate-900 w-full mb-3">
              {expandedSections.assets ? <ChevronDown className="h-3.5 w-3.5" /> : <ChevronRight className="h-3.5 w-3.5" />}
              ...
              <div className="space-y-3">
                ...
                <div>
                  ...
                  <div className="flex items-center justify-between mt-1">
                    <span className="text-[10px] text-slate-400">{design.show_bg_image === false ? "Hidden" : "Visible"}</span>
                    <Switch checked={design.show_bg_image !== false}
          ...
          {/* SVG ICON LIBRARY */}
          <div className="border-t border-slate-100 pt-4">
            <button onClick={() => toggle("svgPicker")} className="flex items-center gap-2 text-xs font-bold text-slate-900 w-full mb-3">
              {expandedSections.svgPicker ? <ChevronDown className="h-3.5 w-3.5" /> : <ChevronRight className="h-3.5 w-3.5" />}
              <Shapes className="h-4 w-4 text-[#6B5BFB]" /> SVG Icon Library
            </button>
            {expandedSections.svgPicker && (
              <div className="space-y-2">
                <Input value={svgSearch} onChange={(e) => setSvgSearch(e.target.value)}
                  placeholder="Search icons..." className="h-8 text-xs" />
                <div className="flex gap-1 overflow-x-auto pb-1">
                  {SVG_CATEGORIES.map((c) => (
                    <button key={c.id} onClick={() => setSvgCategory(c.id)}
                      className={`px-2 py-0.5 rounded text-[10px] font-medium whitespace-nowrap ${
                        svgCategory === c.id ? "bg-[#6B5BFB] text-white" : "bg-slate-100 text-slate-500"
                      }`}>
                      {c.label}
                    </button>
                  ))}
                ...
                <div className="grid grid-cols-5 gap-1 max-h-40 overflow-y-auto">
                  {filteredSvgs.map((icon) => (
                    <button key={icon.id} onClick={() => onAddSvg(icon.svg, icon.name)}
                      ...
                      title={icon.name}
                      dangerouslySetInnerHTML={{ __html: icon.svg.replace(/currentColor/, design.accent_color) }} />
                  ))}
                </div>
              </div>
            )}
