...
export function CertDesignerAdmin() {
  ...
  const handleSeed = async () => {
    try {
      ...
      toast.success(
        `Seeded: ${res.created} SVG templates created${res.errors?.length ? `, ${res.errors.length} errors` : ""}`,
      );
        ...
        {isLoading ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {filtered.map((t) => (
              <Card
                key={t.id}
                className="group overflow-hidden border border-slate-200 hover:border-[#6B5BFB]/40 hover:shadow-xl transition-all duration-300"
              >
          ...
          <div className="text-center py-24 bg-white rounded-2xl border border-slate-200">
            ...
            <p className="text-sm text-slate-500 mt-1 mb-6">
              Seed 155 SVG templates from 8 categories or create your own
            </p>
      ...
      <Dialog open={!!deleteId} onOpenChange={() => setDeleteId(null)}>
        <DialogContent className="sm:max-w-md">
          ...
          <DialogFooter>
            ...
            <Button variant="destructive" onClick={handleDelete} className="text-xs">
              Delete
            </Button>
          </DialogFooter>
        </DialogContent>
