...
import {
  ...
  Eye, EyeOff, Lock, Unlock, Layers, Image as ImageIcon, Square, Ribbon,
  RotateCcw, ChevronDown, ChevronRight,
} from "lucide-react";
import {
  CertElement, CertDesign, FONTS, BORDER_OPTIONS, PATTERN_OPTIONS, THEMES,
  ShapeType, SHAPE_OPTIONS,
} from "./types";
...
export function DesignerSidebar({
  ...
  const [expandedSections, setExpandedSections] = useState<Record<string, boolean>>({
    text: true, position: true, style: true, border: true, assets: true,
  });
        ...
        <ScrollArea className="flex-1">
          <div className="p-4 space-y-6">
            ...
            <TabsContent value="design" className="m-0 space-y-6">
                  ...
                  {THEMES.map((t) => (
                    <button
                      ...
                      onClick={() => onUpdateDesign({
                        accent_color: t.accent, bg_color: t.bg, text_color: t.text,
                        accent_color_2: t.accent2, font_family: t.font, border_style: t.border,
                        border_width: t.borderWidth, corner_style: t.corner,
                        background_pattern: t.pattern, layout: t.layout,
                      })}
              ...
              <div className="space-y-3 border-t pt-4">
                ...
                <div className="grid grid-cols-2 gap-3">
                  {([
                    ...
                    { label: "Accent 2", key: "accent_color_2", value: design.accent_color_2 || "#8a6d2b" },
                  ] as const).map(c => (
                    <div key={c.key}>
              ...
              <div className="space-y-3 border-t pt-4">
                <div className="flex items-center justify-between">
                  ...
                  <Button size="sm" variant={design.show_bg_image === false ? "outline" : "secondary"}
                ...
                <Input value={bgImageUrl} onChange={(e) => onUpdateBgImageUrl(e.target.value)}
                  placeholder="https://example.com/image.png" className="text-xs" />
                {bgImageUrl && (
                  <Button size="sm" variant="ghost" onClick={() => onUpdateBgImageUrl("")}
              ...
              <div className="space-y-3 border-t pt-4">
                ...
                {onAddShape && onAddSvg && onAddDivider && onUploadImage && (
                  <AssetLibrary onAddShape={onAddShape} onAddSvg={onAddSvg}
                    onAddDivider={onAddDivider} onUploadImage={onUploadImage}
            ...
            <TabsContent value="layers" className="m-0 space-y-2">
              ...
              {[...elements].reverse().map((el) => (
                <div key={el.id}
                  className={`flex items-center justify-between p-2 rounded-lg border text-xs transition-colors cursor-pointer ${
                    selectedId === el.id ? "bg-primary/10 border-primary/40 shadow-sm" : "bg-card hover:bg-accent/40 border-border"
                  ...
                  <div className="flex items-center gap-1 shrink-0">
                    ...
                    <Button variant="ghost" size="icon" className="h-6 w-6"
                      onClick={(e) => { e.stopPropagation(); onDuplicateElement(el.id); }} title="Duplicate">
                      <Copy className="h-3 w-3" />
                    </Button>
                    <Button variant="ghost" size="icon" className="h-6 w-6 text-destructive hover:text-destructive"
                      onClick={(e) => { e.stopPropagation(); onDeleteElement(el.id); }} title="Delete">
              ...
              {elements.length === 0 && (
                <p className="text-xs text-muted-foreground text-center py-6">No elements on canvas</p>
              )}
            </TabsContent>
              ...
              {selectedEl ? (
                <>
                  ...
                  {(selectedEl.type === "text") && (
                    <div className="space-y-4">
                      <button onClick={() => toggle("text")} className="flex items-center gap-2 text-xs font-semibold w-full">
                      ...
                      {expandedSections.text && (
                        <>
                          ...
                          <div>
                            ...
                            <FontPicker value={selectedEl.fontFamily || design.font_family}
                            ...
                            <div>
                              ...
                              <div className="flex gap-2 items-center mt-1">
                                <input type="color" value={selectedEl.color || design.text_color}
                                  ...
                                  className="w-8 h-8 rounded cursor-pointer" />
                                <Input value={selectedEl.color || ""}
                                  onChange={(e) => onUpdateElement(selectedEl.id, { color: e.target.value })}
                                  className="h-8 text-xs font-mono" placeholder="inherit" />
                              </div>
                            </div>
                          </div>
                          <div className="grid grid-cols-4 gap-1 bg-muted p-1 rounded-md">
                            <Button variant={selectedEl.align === "left" ? "secondary" : "ghost"} size="sm" className="h-8"
                              onClick={() => onUpdateElement(selectedEl.id, { align: "left" })}>
                              <AlignLeft className="h-4 w-4" />
                            </Button>
                            <Button variant={selectedEl.align === "center" ? "secondary" : "ghost"} size="sm" className="h-8"
                              onClick={() => onUpdateElement(selectedEl.id, { align: "center" })}>
                              <AlignCenter className="h-4 w-4" />
                            </Button>
                            <Button variant={selectedEl.align === "right" ? "secondary" : "ghost"} size="sm" className="h-8"
                              onClick={() => onUpdateElement(selectedEl.id, { align: "right" })}>
                              <AlignRight className="h-4 w-4" />
                            </Button>
                            <Button variant={selectedEl.fontWeight === "bold" ? "secondary" : "ghost"} size="sm" className="h-8"
                              onClick={() => onUpdateElement(selectedEl.id, { fontWeight: selectedEl.fontWeight === "bold" ? "normal" : "bold" })}>
                              <b className="font-serif">B</b>
                            </Button>
                          </div>
                          <div>
                            <Label className="text-xs">Letter Spacing: {selectedEl.letterSpacing || 0}px</Label>
                            <Slider value={[selectedEl.letterSpacing || 0]} min={-5} max={30} step={0.5}
                              onValueChange={(v) => onUpdateElement(selectedEl.id, { letterSpacing: v[0] })} className="mt-1" />
                          </div>
                        </>
                      )}
                    </div>
                  ...
                  {(selectedEl.type === "image" || selectedEl.type === "org_logo" || selectedEl.type === "signature" || selectedEl.type === "badge") && (
                    <div className="space-y-4">
                      <button onClick={() => toggle("style")} className="flex items-center gap-2 text-xs font-semibold w-full">
                        {expandedSections.style ? <ChevronDown className="h-3 w-3" /> : <ChevronRight className="h-3 w-3" />}
                      ...
                      <div>
                        ...
                        <div className="flex gap-2 items-center mt-1">
                          <input type="color" value={selectedEl.svgColor || design.accent_color}
                            ...
                            className="w-8 h-8 rounded cursor-pointer" />
                          <Input value={selectedEl.svgColor || ""}
                            onChange={(e) => onUpdateElement(selectedEl.id, { svgColor: e.target.value })}
                            className="h-8 text-xs font-mono" placeholder="auto" />
                        </div>
                      </div>
                    </div>
                  )}
                  ...
                  {selectedEl.type === "shape" && (
                    <div className="space-y-4">
                      ...
                      <Select value={selectedEl.shapeType || "rectangle"}
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        <div>
                          ...
                          <div className="flex gap-2 items-center mt-1">
                            <input type="color" value={selectedEl.fillColor || "transparent"}
                              onChange={(e) => onUpdateElement(selectedEl.id, { fillColor: e.target.value })}
                              className="w-8 h-8 rounded cursor-pointer" />
                            <Input value={selectedEl.fillColor || ""}
                              onChange={(e) => onUpdateElement(selectedEl.id, { fillColor: e.target.value })}
                              className="h-8 text-xs font-mono" placeholder="none" />
                          </div>
                        </div>
                        <div>
                          ...
                          <div className="flex gap-2 items-center mt-1">
                            <input type="color" value={selectedEl.strokeColor || design.accent_color}
                              ...
                              className="w-8 h-8 rounded cursor-pointer" />
                            <Input value={selectedEl.strokeColor || ""}
                              onChange={(e) => onUpdateElement(selectedEl.id, { strokeColor: e.target.value })}
                              className="h-8 text-xs font-mono" placeholder="auto" />
                          </div>
                        </div>
                      ...
                      <div>
                        <Label className="text-xs">Stroke Width: {selectedEl.strokeWidth || 2}px</Label>
                        <Slider value={[selectedEl.strokeWidth || 2]} min={0} max={20}
                          onValueChange={(v) => onUpdateElement(selectedEl.id, { strokeWidth: v[0] })} className="mt-1" />
                      </div>
                    </div>
                  )}
                  ...
                  {selectedEl.type === "date" && (
                    <div className="space-y-4">
                      ...
                      <Select value={selectedEl.dateFormat || "YYYY-MM-DD"}
                      ...
                      <div>
                        ...
                        <Input value={selectedEl.label || ""}
                          ...
                          placeholder="Issued Date" className="mt-1 text-xs" />
                      </div>
                    </div>
                  )}
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        <div>
                          ...
                          <Input type="number" value={selectedEl.rows || 3} min={1} max={20}
                            onChange={(e) => onUpdateElement(selectedEl.id, { rows: parseInt(e.target.value) || 3 })}
                            className="mt-1 h-8 text-xs" />
                        </div>
                        <div>
                          <Label className="text-xs">Columns</Label>
                          <Input type="number" value={selectedEl.cols || 2} min={1} max={10}
                            onChange={(e) => onUpdateElement(selectedEl.id, { cols: parseInt(e.target.value) || 2 })}
                            className="mt-1 h-8 text-xs" />
                        </div>
                      </div>
                      <div>
                        <Label className="text-xs">Cell Padding: {selectedEl.cellPadding || 4}px</Label>
                        <Slider value={[selectedEl.cellPadding || 4]} min={0} max={20}
                          onValueChange={(v) => onUpdateElement(selectedEl.id, { cellPadding: v[0] })} className="mt-1" />
                      </div>
                    </div>
                  )}
                  ...
                  {/* POSITION & SIZE */}
                  <div className="space-y-4">
                    <button onClick={() => toggle("position")} className="flex items-center gap-2 text-xs font-semibold w-full">
                      {expandedSections.position ? <ChevronDown className="h-3 w-3" /> : <ChevronRight className="h-3 w-3" />}
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        ...
                        <div>
                          ...
                          <Input type="number" value={Math.round(selectedEl.width || 0)}
                            ...
                            placeholder="Auto" className="h-7 text-xs font-mono mt-1" />
                        </div>
                        <div>
                  ...
                  <div className="border-t pt-4 grid grid-cols-2 gap-2">
                    <Button variant="outline" size="sm" onClick={() => onDuplicateElement(selectedEl.id)}>
                      <Copy className="h-4 w-4 mr-2" /> Duplicate
                    </Button>
                    <Button variant="destructive" size="sm" onClick={() => onDeleteElement(selectedEl.id)}>
