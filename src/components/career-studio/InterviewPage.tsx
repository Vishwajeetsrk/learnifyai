...
const PRESET_QUESTIONS: Record<string, Question[]> = {
  mobile: [
    {
      question: "What are the key differences between Android and iOS development, and how do you decide which platform to develop for first?",
      type: "technical",
      tips: [
  ...
  frontend: [
    {
      question: "Explain the virtual DOM reconciliation process in React, and how Fiber architecture improves UI responsiveness.",
      type: "technical",
      tips: [
  ...
  return (
    <div className="flex flex-col items-center justify-center p-4">
      <svg className="w-24 h-24 drop-shadow-xl" viewBox="0 0 100 100" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        {/* Head/Face */}
        <path d="M50 25 C36 25 33 36 33 48 C33 60 38 70 50 70 C62 70 67 60 67 48 C67 36 64 25 50 25 Z" fill="#FDBA74" />
        
        {/* Hair */}
        {isSarah ? (
          /* Female/Sarah Hair Style */
          <>
            <path d="M50 22 C34 22 30 30 30 45 C35 44 40 36 50 36 C60 36 65 44 70 45 C70 30 66 22 50 22 Z" fill="#451a03" />
          ...
          <path d="M50 21 C36 21 34 26 34 32 C38 32 44 26 50 28 C56 26 62 32 66 32 C66 26 64 21 50 21 Z" fill="#1e293b" />
        )}
        
        {/* Eyes (Blinking Animation via CSS Class) */}
        <ellipse cx="43" cy="46" rx="3" ry="3" fill="#1E293B" className="animate-blink" style={{ transformOrigin: "43px 46px" }} />
        <ellipse cx="57" cy="46" rx="3" ry="3" fill="#1E293B" className="animate-blink" style={{ transformOrigin: "57px 46px" }} />
        ...
        {isSarah && (
          <>
            <rect x="37" y="43" width="11" height="7" rx="2" stroke="#0f172a" strokeWidth="1.2" fill="none" />
            <rect x="52" y="43" width="11" height="7" rx="2" stroke="#0f172a" strokeWidth="1.2" fill="none" />
            <line x1="48" y1="46" x2="52" y2="46" stroke="#0f172a" strokeWidth="1.2" />
          </>
        )}

        <defs>
          <linearGradient id="sarah-grad" x1="0" y1="0" x2="100" y2="100" gradientUnits="userSpaceOnUse">
            <stop stopColor="#6366F1" />
            <stop offset="1" stopColor="#EC4899" />
          </linearGradient>
          <linearGradient id="alex-grad" x1="0" y1="0" x2="100" y2="100" gradientUnits="userSpaceOnUse">
...
export function InterviewPage({ embedded = false }: { embedded?: boolean }) {
  ...
  useEffect(() => {
    if (step === "interview" && (mode === "video" || mode === "voice") && typeof window !== "undefined") {
  ...
  const loadQuestion = useCallback(
    async (idx: number) => {
      ...
      const presets = PRESET_QUESTIONS[roleKey] || PRESET_QUESTIONS[roleKey.includes("frontend") ? "frontend" : ""];
        ...
        <div className="space-y-6">
          ...
          <div className="rounded-2xl border bg-card p-6 space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3 max-w-md">
              <button
                ...
                className={cn("p-4 rounded-xl border text-center transition-all cursor-pointer", avatarModel === "sarah" ? "border-primary bg-primary/5 ring-1 ring-primary" : "border-border hover:bg-muted/30")}
              >
                <div className="w-12 h-12 rounded-full mx-auto bg-gradient-to-br from-indigo-500 to-pink-500 flex items-center justify-center text-white font-bold mb-2">SJ</div>
              ...
              <button
                ...
                className={cn("p-4 rounded-xl border text-center transition-all cursor-pointer", avatarModel === "alex" ? "border-primary bg-primary/5 ring-1 ring-primary" : "border-border hover:bg-muted/30")}
              >
                <div className="w-12 h-12 rounded-full mx-auto bg-gradient-to-br from-blue-500 to-emerald-500 flex items-center justify-center text-white font-bold mb-2">AR</div>
          ...
          <div className="rounded-2xl border bg-card p-6">
            ...
            <div className="flex flex-wrap gap-3">
              {DIFFICULTIES.map((d) => (
                <button key={d.id} onClick={() => setDifficulty(d.id as any)} className={cn("px-6 py-3 rounded-xl border font-medium transition-all", difficulty === d.id ? "border-primary bg-primary/5 shadow-sm" : "border-border hover:border-primary/40")}>
        ...
        <div className="space-y-6">
          <div className="flex items-center gap-3">
            <div className="flex-1 h-2 bg-muted rounded-full overflow-hidden">
              <div className="h-full bg-primary rounded-full transition-all" style={{ width: `${((questionIndex + 1) / totalQuestions) * 100}%` }} />
            </div>
            <span className="text-xs sm:text-sm text-muted-foreground font-bold shrink-0">{questionIndex + 1} / {totalQuestions}</span>
            <Button
              size="sm"
              variant="outline"
            ...
            >
              <XCircle className="h-3.5 w-3.5" /><span>Exit / Cancel</span>
            </Button>
          </div>
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="rounded-2xl border border-indigo-500/30 bg-gradient-to-br from-indigo-950/40 via-card to-purple-950/30 p-5 flex flex-col items-center justify-center relative overflow-hidden shadow-lg min-h-[220px]">
                  <div className="absolute top-3 left-3 flex items-center gap-1.5 bg-indigo-500/20 text-indigo-400 border border-indigo-500/30 px-2.5 py-0.5 rounded-full text-[10px] font-bold backdrop-blur-md">
                    <span className={cn("w-2 h-2 rounded-full bg-indigo-400", aiSpeaking && "animate-ping")} />
                ...
                <div className="rounded-2xl border border-slate-800 bg-slate-950 p-3 flex flex-col justify-between relative overflow-hidden shadow-lg min-h-[220px]">
                  ...
                  <div className="w-full flex-1 flex flex-col items-center justify-center relative rounded-xl overflow-hidden bg-slate-900 border border-slate-800" style={{ minHeight: 140 }}>
                    ...
                    {(!cameraActive || mode !== "video") && (
                      <div className="flex flex-col items-center justify-center p-4 text-center">
                        ...
                        <span className="text-[10px] text-slate-500 mt-0.5">Microphone connected & ready</span>
                      </div>
                    )}
                  </div>
                  ...
                  <div className="w-full flex items-center justify-between pt-2.5 border-t border-slate-800 mt-2 text-xs">
                    <div className="flex gap-2">
                      <Button size="sm" variant="ghost" onClick={toggleMic} className="h-7 w-7 p-0 rounded-full bg-slate-800 hover:bg-slate-700 text-slate-200">
                        {micActive ? <Mic className="h-3.5 w-3.5" /> : <MicOff className="h-3.5 w-3.5 text-red-400" />}
                      </Button>
                      {mode === "video" && (
                        <Button size="sm" variant="ghost" onClick={toggleCamera} className="h-7 w-7 p-0 rounded-full bg-slate-800 hover:bg-slate-700 text-slate-200">
                          {cameraActive ? <Video className="h-3.5 w-3.5" /> : <VideoOff className="h-3.5 w-3.5 text-red-400" />}
                        </Button>
                      )}
                    </div>
              ...
              <div className="rounded-2xl border bg-card p-6">
                <div className="flex items-start justify-between mb-3">
                  ...
                  <div className="flex gap-2">
                    <Button variant="ghost" size="sm" onClick={() => setShowTips(!showTips)} className="text-xs">
                      <Lightbulb className="h-3.5 w-3.5 mr-1" /> Tips
                    </Button>
                    {(mode === "voice" || mode === "video") && (
                      <Button variant="ghost" size="sm" onClick={() => setIsMuted(!isMuted)} className="text-xs">
                ...
                {showTips && currentQuestion.tips?.length > 0 && (
                  <div className="mt-4 p-3 rounded-lg bg-primary/5 border border-primary/10">
                    ...
                    <ul className="text-sm text-muted-foreground space-y-1">
                      {currentQuestion.tips.map((tip, i) => <li key={i}>• {tip}</li>)}
                    </ul>
                  </div>
                )}
              </div>
              ...
              {!evaluation && (
                <div className="rounded-2xl border bg-card p-6 space-y-4">
                  {mode === "chat" ? (
                    <Textarea ref={textareaRef} value={userAnswer} onChange={(e) => setUserAnswer(e.target.value)} placeholder="Type your answer here..." rows={6} className="resize-none" />
                  ) : (
                    <div className="space-y-3">
                      <div className="flex items-center gap-3">
                        <Button variant={isRecording ? "destructive" : "default"} size="lg" onClick={isRecording ? stopVoiceRecognition : startVoiceRecognition} className="rounded-full h-14 w-14">
                          {isRecording ? <MicOff className="h-6 w-6" /> : <Mic className="h-6 w-6" />}
                        </Button>
                        <div>
                          <p className="text-sm font-medium">{isRecording ? "Recording... Click to stop" : "Click to start recording"}</p>
                          <p className="text-xs text-muted-foreground">Speak your answer clearly</p>
                        </div>
                      </div>
                      {userAnswer && <div className="p-3 rounded-lg bg-muted/50 text-sm">{userAnswer}</div>}
                    </div>
                  )}

                  <Button onClick={handleSubmitAnswer} disabled={!userAnswer.trim() || evaluating} className="w-full">
                    {evaluating ? <Loader2 className="h-4 w-4 animate-spin mr-2" /> : <Send className="h-4 w-4 mr-2" />}
                ...
                <div className="rounded-2xl border bg-card p-6 space-y-4">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                      <span className={cn("text-2xl font-bold", evaluation.score >= 70 ? "text-emerald-500" : evaluation.score >= 50 ? "text-yellow-500" : "text-red-500")}>{evaluation.score}</span>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    {evaluation.strengths?.length > 0 && (
                      <div>
                        <p className="text-xs font-medium text-emerald-500 mb-1.5 flex items-center gap-1"><CheckCircle2 className="h-3 w-3" /> Strengths</p>
                        <ul className="text-sm space-y-1">
                          {evaluation.strengths.map((s, i) => <li key={i} className="text-muted-foreground">• {s}</li>)}
                        </ul>
                      </div>
                    )}
                    {evaluation.improvements?.length > 0 && (
                      <div>
                        <p className="text-xs font-medium text-yellow-500 mb-1.5 flex items-center gap-1"><XCircle className="h-3 w-3" /> Improvements</p>
                        <ul className="text-sm space-y-1">
                          {evaluation.improvements.map((s, i) => <li key={i} className="text-muted-foreground">• {s}</li>)}
                        </ul>
                      </div>
                    )}
                  </div>
        ...
        <div className="space-y-6">
          <div className="rounded-2xl border bg-card p-8 text-center">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 max-w-md mx-auto mb-6">
              ...
              <div className="p-4 rounded-xl bg-muted/50">
                <p className={cn("text-3xl font-bold", avgScore >= 70 ? "text-emerald-500" : avgScore >= 50 ? "text-yellow-500" : "text-red-500")}>
            ...
            <Button onClick={handleRestart}><RotateCcw className="h-4 w-4 mr-1.5" /> New Interview</Button>
          </div>
            ...
            <div className="space-y-2">
              {scores.map((score, i) => (
                <div key={i} className="flex items-center gap-3">
                  ...
                  <div className="flex-1 h-2 bg-muted rounded-full overflow-hidden">
                    <div className={cn("h-full rounded-full transition-all", score >= 70 ? "bg-emerald-500" : score >= 50 ? "bg-yellow-500" : "bg-red-500")} style={{ width: `${score}%` }} />
                  </div>
